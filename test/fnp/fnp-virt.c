...
#define VIRT_DEMO_DEFAULT_TUN_DEV "tun0"
#define VIRT_DEMO_DEFAULT_TUN_LOCAL_IP "10.77.0.2"
#define VIRT_DEMO_DEFAULT_TAP_DEV "tap0"
#define VIRT_DEMO_DEFAULT_TAP_LOCAL_IP "10.88.0.2"
#define VIRT_DEMO_DEFAULT_TAP_LOCAL_MAC "02:00:00:00:10:02"
#define VIRT_DEMO_MAX_PKT_LEN 2048
...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            "\n"
            "Defaults:\n"
            "  tun-serve dev=%s service_ip=%s\n"
            "  tap-serve dev=%s service_mac=%s service_ip=%s\n",
            prog, prog,
            VIRT_DEMO_DEFAULT_TUN_DEV, VIRT_DEMO_DEFAULT_TUN_LOCAL_IP,
            VIRT_DEMO_DEFAULT_TAP_DEV, VIRT_DEMO_DEFAULT_TAP_LOCAL_MAC, VIRT_DEMO_DEFAULT_TAP_LOCAL_IP);
}
...
static int run_tun_serve(const char *dev_name, const char *service_ip_text)
{
    ...
    if (service_ip_be == 0)
    {
        fprintf(stderr, "invalid tun service ip: %s\n", service_ip_text);
        return FNP_ERR_PARAM;
    }
    ...
    if (ret != FNP_OK)
    {
        fprintf(stderr, "fail to create tun socket: %d\n", ret);
        return ret;
    }

    printf("tun serve on dev=%s service_ip=%s\n", conf.dev_name, service_ip_text);
    u8 packet[VIRT_DEMO_MAX_PKT_LEN];
    while (1)
    {
        ...
        u8 reply_packet[VIRT_DEMO_MAX_PKT_LEN];
...
static int run_tap_serve(const char *dev_name, const char *service_mac_text, const char *service_ip_text)
{
    ...
    {
        fprintf(stderr, "invalid tap service mac: %s\n", service_mac_text);
        return FNP_ERR_PARAM;
    }
    ...
    if (service_ip_be == 0)
    {
        fprintf(stderr, "invalid tap service ip: %s\n", service_ip_text);
        return FNP_ERR_PARAM;
    }
    ...
    if (ret != FNP_OK)
    {
        fprintf(stderr, "fail to create tap socket: %d\n", ret);
        return ret;
    }
    ...
    u8 frame[VIRT_DEMO_MAX_PKT_LEN];
    while (1)
    {
        ...
        if (ether_type == RTE_ETHER_TYPE_ARP)
        {
            ...
            format_ipv4(arp->arp_data.arp_sip, sender_ip, sizeof(sender_ip));
            format_mac(&arp->arp_data.arp_sha, sender_mac, sizeof(sender_mac));
            printf("tap recv arp request from %s (%s)\n", sender_ip, sender_mac);

            send_tap_arp_reply(socket,
                               &service_mac,
                               service_ip_be,
                               &arp->arp_data.arp_sha,
                               arp->arp_data.arp_sip);
            continue;
        }
        ...
        printf("tap recv udp %s/%s:%u -> %s:%u, %d bytes: %.*s\n",
               ...
               payload_len,
               payload_len,
               (char *)((u8 *)udp + sizeof(*udp)));

        u8 reply_frame[VIRT_DEMO_MAX_PKT_LEN];
...
int main(int argc, char **argv)
{
    ...
    {
        const char *dev_name = argc > 2 ? argv[2] : VIRT_DEMO_DEFAULT_TUN_DEV;
        const char *service_ip = argc > 3 ? argv[3] : VIRT_DEMO_DEFAULT_TUN_LOCAL_IP;
        return run_tun_serve(dev_name, service_ip);
    }

    if (strcmp(argv[1], "tap-serve") == 0)
    {
        const char *dev_name = argc > 2 ? argv[2] : VIRT_DEMO_DEFAULT_TAP_DEV;
        const char *service_mac = argc > 3 ? argv[3] : VIRT_DEMO_DEFAULT_TAP_LOCAL_MAC;
        const char *service_ip = argc > 4 ? argv[4] : VIRT_DEMO_DEFAULT_TAP_LOCAL_IP;
