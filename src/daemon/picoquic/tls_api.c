...
int picoquic_set_cipher_suite(quic_context_t* quic, int cipher_suite_id)
{
...
int picoquic_set_key_exchange(quic_context_t* quic, int key_exchange_id)
{
    int ret = 0;
    ptls_context_t* ctx = (ptls_context_t*)quic->tls_master_ctx;
...
int picoquic_set_private_key_from_file(quic_context_t* quic, char const* file_name)
{
...
ptls_verify_certificate_t* picoquic_get_certificate_verifier(char const* cert_root_file_name,
                                                             ...
                                                             picoquic_free_verify_certificate_ctx*
...
int picoquic_set_tls_root_certificates(quic_context_t* quic, ptls_iovec_t* certs, size_t count)
{
    int ret = -1;
...
static void picoquic_free_log_event(quic_context_t* quic);

void picoquic_log_crypto_errors(quic_cnx_t* cnx, int ret)
{
...
int picoquic_server_setup_ticket_aead_contexts(quic_context_t* quic,
                                               ptls_context_t* tls_ctx,
                                               const uint8_t* secret, size_t secret_length);

/* Crypto random number generator */

void picoquic_crypto_random(quic_context_t* quic, void* buf, size_t len)
{
    ptls_context_t* ctx = (ptls_context_t*)quic->tls_master_ctx;
...
uint64_t picoquic_crypto_uniform_random(quic_context_t* quic, uint64_t rnd_max)
{
    uint64_t rnd;
...
void picoquic_public_random_seed(quic_context_t* quic)
{
    uint64_t seed[3];
...
uint16_t picoquic_tls_get_quic_extension_id(quic_cnx_t* cnx)
{
    int v = picoquic_supported_versions[cnx->version_index].version;
    return PICOQUIC_TRANSPORT_PARAMETERS_TLS_EXTENSION_V1;
}
...
void picoquic_tls_set_extensions(quic_cnx_t* cnx, picoquic_tls_ctx_t* tls_ctx)
{
    size_t consumed = 0;
    int ret = -1;
                                         ...
                                         ptls_raw_extension_t* slots)
{
    ...
    size_t consumed = 0;
    /* Find the context from the TLS context */
    picoquic_tls_ctx_t* ctx = (picoquic_tls_ctx_t*)((char*)properties - offsetof(
        struct st_picoquic_tls_ctx_t, handshake_properties));
                                    ...
                                    ptls_t* tls, ptls_on_client_hello_parameters_t* params)
{
    ...
    int ret = 0;
    quic_context_t** ppquic = (quic_context_t**)(((char*)on_hello_cb_ctx) + sizeof(ptls_on_client_hello_t));
    quic_context_t* quic = *ppquic;
    ...
    if (quic->default_alpn != NULL)
    {
        ...
        {
            if (params->negotiated_protocols.list[i].len == len && memcmp(
                params->negotiated_protocols.list[i].base, quic->default_alpn, len) == 0)
            {
                if (quic->cnx_in_progress != NULL)
                {
                    picoquic_log_app_message(quic->cnx_in_progress, "ALPN[%d] matches default alpn (%s)", (int)i,
                                             quic->default_alpn);
                }
    ...
    {
        size_t selected = quic->alpn_select_fn(quic, params->negotiated_protocols.list,
        ...
        if (selected < params->negotiated_protocols.count)
        {
            ...
            ptls_set_negotiated_protocol(tls, (const char*)params->negotiated_protocols.list[selected].base,
                                         params->negotiated_protocols.list[selected].len);
        }
    }
    ...
    if (quic->cnx_in_progress != NULL)
    {
        ...
        picoquic_log_negotiated_alpn(quic->cnx_in_progress,
                                     0, params->server_name.base, params->server_name.len, alpn_found,
                                     alpn_found_length,
                                             ...
                                             ptls_t* tls, int is_encrypt, ptls_buffer_t* dst, ptls_iovec_t src)
{
     ...
     */
    int ret = 0;
    quic_context_t** ppquic = (quic_context_t**)(((char*)encrypt_ticket_ctx) + sizeof(ptls_encrypt_ticket_t));
    quic_context_t* quic = *ppquic;
            ...
            else
            {
                ...
                if (version_number != picoquic_supported_versions[quic->cnx_in_progress->version_index].version)
                {
                    ...
                    picoquic_log_app_message(quic->cnx_in_progress, "Ticket version mismatch, expected 0x%x, got 0x%x",
                                             picoquic_supported_versions[quic->cnx_in_progress->version_index].version,
                                             version_number);
                }
                else
                {
...
{
    int ret = 0;
    quic_context_t* quic = *((quic_context_t**)(((char*)save_ticket_ctx) + sizeof(ptls_save_ticket_t)));
    const char* sni = ptls_get_server_name(tls);
    const char* alpn = ptls_get_negotiated_protocol(tls);
    quic_cnx_t* cnx = (quic_cnx_t*)*ptls_get_data_ptr(tls);
...
#endif
void picoquic_dispose_verify_certificate_callback(quic_context_t* quic)
{
    ptls_context_t* ctx = (ptls_context_t*)quic->tls_master_ctx;
...
void picoquic_tls_set_verify_certificate_callback(quic_context_t* quic,
...
static int picoquic_set_key_from_secret(ptls_cipher_suite_t* cipher, int is_enc, int is_rotation,
...
static int picoquic_update_traffic_key_callback(ptls_update_traffic_key_t* self, ptls_t* tls, int is_enc, size_t epoch,
                                                const void* secret)
{
    quic_cnx_t* cnx = (quic_cnx_t*)*ptls_get_data_ptr(tls);
...
static int picoquic_compute_initial_secrets(quic_context_t* quic, int version_index,
                                            quic_connection_id_t* initial_cnxid,
                                            ptls_cipher_suite_t** cipher, uint8_t* client_secret,
                                            uint8_t* server_secret)
{
    int ret = 0;
    ptls_iovec_t salt;
...
int picoquic_setup_initial_traffic_keys(quic_cnx_t* cnx)
{
    int ret = 0;
    const char* prefix_label = picoquic_supported_versions[cnx->version_index].tls_prefix_label;
    ...
    ret = picoquic_compute_initial_secrets(cnx->quic, cnx->version_index, &cnx->initial_cnxid, &cipher, client_secret,
                                           server_secret);
...
int picoquic_get_initial_aead_context(
    quic_context_t* quic, quic_packet_header* ph,
    int is_client, int is_enc, void** aead_ctx, void** pn_enc_ctx)
{
    ...
    uint8_t client_secret[256];
    uint8_t server_secret[256];
    int version_index = ph->version_index;
    quic_connection_id_t* initial_cnxid = &ph->dest_cnx_id;
    ...
    *pn_enc_ctx = NULL;
    ...
    if (!is_client)
    {
        // 对于服务端，加密使用server_secret，解密使用client_secret
        selected_secret = (is_enc) ? server_secret : client_secret;
    }
    else
    {
        // 对于客户端，加密使用client_secret，解密使用server_secret
...
uint8_t* picoquic_get_app_secret(quic_cnx_t* cnx, int is_enc)
{
    picoquic_tls_ctx_t* tls_ctx = (picoquic_tls_ctx_t*)cnx->tls_ctx;
...
size_t picoquic_get_app_secret_size(quic_cnx_t* cnx)
{
    picoquic_tls_ctx_t* tls_ctx = (picoquic_tls_ctx_t*)cnx->tls_ctx;
...
int picoquic_compute_new_rotated_keys(quic_cnx_t* cnx)
{
    int ret = 0;
    picoquic_tls_ctx_t* tls_ctx = (picoquic_tls_ctx_t*)cnx->tls_ctx;
    ptls_cipher_suite_t* cipher = ptls_get_cipher(tls_ctx->tls);
...
void picoquic_apply_rotated_keys(quic_cnx_t* cnx, int is_enc)
{
    if (is_enc)
    {
...
int picoquic_master_tlscontext(quic_context_t* quic,
    ...
    else
    {
        ...
        if (ret == 0)
        {
            ...
            if (cert_file_name != NULL && key_file_name != NULL)
            {
                ...
                {
                    printf("Cannot load certificate: %s", cert_file_name);
                    ret = -1;
                }
                else
                {
        ...
        if (ret == 0)
        {
            och = (ptls_on_client_hello_t*)malloc(sizeof(ptls_on_client_hello_t) + sizeof(quic_context_t*));
            if (och != NULL)
            {
                quic_context_t** ppquic = (quic_context_t**)(((char*)och) + sizeof(ptls_on_client_hello_t));
        ...
        if (ret == 0)
        {
            encrypt_ticket = (ptls_encrypt_ticket_t*)malloc(sizeof(ptls_encrypt_ticket_t) + sizeof(quic_context_t*));
            ...
            {
                quic_context_t** ppquic = (quic_context_t**)(((char*)encrypt_ticket) + sizeof(ptls_encrypt_ticket_t));
        ...
        if (ret == 0)
        {
            ctx->verify_certificate = picoquic_get_certificate_verifier(cert_root_file_name,
                                                                        ...
                                                                        (picoquic_free_verify_certificate_ctx*)&quic->
        ...
        {
            save_ticket = (ptls_save_ticket_t*)malloc(sizeof(ptls_save_ticket_t) + sizeof(quic_context_t*));
            if (save_ticket != NULL)
            {
                quic_context_t** ppquic = (quic_context_t**)(((char*)save_ticket) + sizeof(ptls_save_ticket_t));
...
void picoquic_master_tlscontext_free(quic_context_t* quic)
{
...
uint64_t picoquic_get_tls_time(quic_context_t* quic)
{
    ptls_context_t* ctx = (ptls_context_t*)quic->tls_master_ctx;
 ...
 */
int picoquic_tlscontext_create(quic_context_t* quic, quic_cnx_t* cnx, uint64_t current_time)
{
    int ret = 0;
 ...
 */
void picoquic_set_key_log_file(quic_context_t* quic, char const* keylog_filename)
{
    ptls_context_t* ctx = (ptls_context_t*)quic->tls_master_ctx;
 ...
 * used during the TLS handshake.
 */
void picoquic_tlscontext_remove_ticket(quic_cnx_t* cnx)
{
...
void picoquic_tlscontext_trim_after_handshake(quic_cnx_t* cnx)
{
    picoquic_tls_ctx_t* ctx = (picoquic_tls_ctx_t*)cnx->tls_ctx;
...
char const* picoquic_tls_get_negotiated_alpn(quic_cnx_t* cnx)
{
    picoquic_tls_ctx_t* ctx = (picoquic_tls_ctx_t*)cnx->tls_ctx;
...
char const* picoquic_tls_get_sni(quic_cnx_t* cnx)
{
    picoquic_tls_ctx_t* ctx = (picoquic_tls_ctx_t*)cnx->tls_ctx;
...
int picoquic_tls_is_psk_handshake(quic_cnx_t* cnx)
{
...
static int picoquic_add_to_tls_stream(quic_cnx_t* cnx, int epoch, const uint8_t* data, size_t length)
{
    quic_stream_t* stream = &cnx->tls_stream[epoch];

    if (length > 0)
    {
        quic_stream_data_t* stream_data = quic_stream_data_alloc();
        if (stream_data == NULL)
        {
            return -1;
        }

        fnp_memcpy(stream_data->bytes, data, length);
        stream_data->length = length;
        stream_data->offset = stream->sent_offset;
...
int picoquic_initialize_tls_stream(quic_cnx_t* cnx, uint64_t current_time)
{
    int ret = 0;
    ...
    if (cnx->sni != NULL && cnx->alpn != NULL && !cnx->quic->client_zero_share)
    {
        picoquic_stored_ticket_t* stored_ticket = picoquic_get_stored_ticket(cnx->quic,
                                                                             cnx->sni, (uint16_t)strlen(cnx->sni),
                                                                             cnx->alpn, (uint16_t)strlen(cnx->alpn),
                                                                             picoquic_supported_versions[cnx->
                                                                                 version_index].version, 1, 0);
    ...
    else
    {
        ...
        if ((ret == 0 || ret == PTLS_ERROR_IN_PROGRESS))
        {
            if (sendbuf.off > 0)
            {
                ret = picoquic_add_to_tls_stream(cnx, 0, sendbuf.base, sendbuf.off);
            }
            else
            {
                ret = 0;
                                               ...
                                               const uint8_t* secret, size_t secret_length)
{
    ...
    else
    {
        if (secret != NULL && secret_length > 0)
        {
            ...
            memcpy(temp_secret, secret,
                   (secret_length > cipher->hash->digest_size) ? cipher->hash->digest_size : secret_length);
        }
        else
        {
                                     ...
                                     void* aead_ctx)
{
    ...
    return ptls_aead_decrypt((ptls_aead_context_t*)aead_ctx,
...
size_t picoquic_aead_decrypt_mp(uint8_t* output, const uint8_t* input, size_t input_length,
                                uint64_t path_id, uint64_t seq_num, const uint8_t* auth_data, size_t auth_data_length,
                                void* aead_context)
{
    size_t decrypted = 0;
...
static void picoquic_setup_cleartext_aead_salt(size_t version_index, ptls_iovec_t* salt)
{
    if (picoquic_supported_versions[version_index].version_aead_key != NULL && picoquic_supported_versions[
        version_index].version_aead_key_length > 0)
    {
...
int picoquic_tls_stream_process(quic_cnx_t* cnx, int* data_consumed, uint64_t current_time)
{
    int ret = 0;
    picoquic_tls_ctx_t* ctx = (picoquic_tls_ctx_t*)cnx->tls_ctx;
    size_t next_epoch = 0;
    ...
    {
        quic_stream_t* stream = &cnx->tls_stream[epoch];
        quic_stream_data_t* data = quic_stream_first_incoming_data(stream);
        size_t processed = 0;
        int data_pushed = 0;
        ...
        if (epoch != next_epoch)
        {
            ...
            else
            {
                if (data != NULL && data->offset > stream->consumed_offset)
                {
                    ...
                    printf("Connection error - TLS data at epoch %d, expected %d.\n",
                           epoch, next_epoch);
        ...
        {
            struct st_ptls_buffer_t sendbuf;
            size_t offset = stream->consumed_offset - data->offset;
            data->bytes += offset;
            data->length -= offset;
            int epoch_data = data->length;
            if (epoch_data > 0)
            {
                ...
                ret = ptls_handle_message(ctx->tls, &sendbuf, send_offset, epoch,
                                          data->bytes, epoch_data, &ctx->handshake_properties);
                    ...
                    ret == PTLS_ERROR_STATELESS_RETRY))
                {
                    for (int i = 0; i < PICOQUIC_NUMBER_OF_EPOCHS; i++)
                    {
                        ...
                        {
                            data_pushed = 1;
                            ret = picoquic_add_to_tls_stream(cnx, i,
                                                             sendbuf.base + send_offset[i],
                                                             send_offset[i + 1] - send_offset[i]);
                        }
                    }
                    if (cnx->client_mode)
                    {
                        if (cnx->alpn == NULL)
                        {
                            ...
                            if (alpn != NULL)
                            {
                                ...
                                picoquic_log_negotiated_alpn(cnx, 0, NULL, 0, (const uint8_t*)alpn, strlen(alpn), NULL,
                                                             0);

                                // if (cnx->callback_fn != NULL)
                                // {
                                //     cnx->callback_fn(cnx, 0, (uint8_t*)alpn, 0, picoquic_callback_set_alpn,
            ...
            if (data->length <= 0)
            {
                quic_stream_dequeue_incoming_data(stream, data);
                data = quic_stream_first_incoming_data(stream);
            }
        }
        ...
        if (processed > 0)
        {
            if (ret == 0)
            {
                ...
                case picoquic_state_client_handshake_start:
                    if (ptls_handshake_is_complete(ctx->tls))
                    {
                        ...
                        {
                            printf("%s", "Connection error - no transport parameter received.\n");
                ...
                case picoquic_state_server_handshake:
                    ...
                    */
                    if (data_pushed == 0 && ((ptls_context_t*)cnx->quic->tls_master_ctx)->require_client_authentication
                        == 1)
                    {
            ...
            else if (ret == PTLS_ERROR_IN_PROGRESS && (cnx->cnx_state == picoquic_state_client_init || cnx->cnx_state ==
                picoquic_state_client_init_sent || cnx->cnx_state == picoquic_state_client_init_resent))
            {
            ...
            else
            {
                ...
                printf("Handshake failed, ret = 0x%x.\n", ret);
 ...
 */
int picoquic_is_tls_complete(quic_cnx_t* cnx)
{
    picoquic_tls_ctx_t* ctx = (picoquic_tls_ctx_t*)cnx->tls_ctx;
...
int picoquic_create_cnxid_reset_secret(quic_context_t* quic, quic_connection_id_t* cnx_id,
                                       uint8_t reset_secret[PICOQUIC_RESET_SECRET_SIZE])
{
    ...
    else
    {
        ...
        else
        {
            ...
            hash_ctx->update(hash_ctx, cnx_id, sizeof(quic_connection_id_t));
...
void picoquic_set_tls_certificate_chain(quic_context_t* quic, ptls_iovec_t* certs, size_t count)
{
    ptls_context_t* ctx = (ptls_context_t*)quic->tls_master_ctx;
...
void picoquic_tls_set_client_authentication(quic_context_t* quic, int client_authentication)
{
    ((ptls_context_t*)quic->tls_master_ctx)->require_client_authentication = client_authentication;
}

int picoquic_tls_client_authentication_activated(quic_context_t* quic)
{
...
static int picoquic_server_encrypt_retry_token(quic_context_t* quic, const struct sockaddr* addr_peer,
                                               int is_new_token, uint8_t* token, size_t* token_length,
                                               size_t token_max, const uint8_t* text, size_t text_length)
{
    int ret = 0;
    uint64_t sequence;
    uint8_t* auth_data;
    ...
    else
    {
        ...
        *token_length = (size_t)8u + picoquic_aead_encrypt_generic(token + 8, text, text_length,
                                                                   sequence, auth_data, auth_data_length,
...
int picoquic_server_decrypt_retry_token(quic_context_t* quic, const struct sockaddr* addr_peer,
                                        int* is_new_token, const uint8_t* token, size_t token_length, uint8_t* text,
                                        size_t* text_length)
{
    int ret = 0;
    uint64_t sequence;
...
int picoquic_prepare_retry_token(quic_context_t* quic, const struct sockaddr* addr_peer,
                                 uint64_t current_time, const quic_connection_id_t* odcid,
                                 const quic_connection_id_t* rcid,
                                 uint32_t initial_pn,
...
int picoquic_verify_retry_token(quic_context_t* quic, const struct sockaddr* addr_peer,
                                uint64_t current_time, int* is_new_token, quic_connection_id_t* odcid,
                                const quic_connection_id_t* rcid,
                                uint32_t initial_pn,
    ...
    if (ret == 0)
    {
            ...
            (bytes = picoquic_frames_varint_decode(bytes, bytes_max, &token_pn)) != NULL)
        {
            ...
            else
            {
                ...
                if (check_reuse && (ret = picoquic_registered_token_check_reuse(quic, token, token_size, token_time)) !=
                    0)
                {
...
void* picoquic_find_retry_protection_context(quic_context_t* quic, int version_index, int sending)
{
    void* aead_ctx = NULL;
        ...
        if (aead_vector != NULL)
        {
            ...
            if (aead_ctx == NULL)
            {
                aead_ctx = picoquic_create_retry_protection_context(
                    sending, picoquic_supported_versions[version_index].version_retry_key,
...
void picoquic_delete_retry_protection_contexts(quic_context_t* quic)
{
...
static size_t picoquic_format_retry_protection_pseudo_packet(uint8_t* pseudo_packet, uint8_t* bytes, size_t byte_index,
                                                             const quic_connection_id_t* odcid)
{
...
size_t picoquic_encode_retry_protection(void* integrity_aead, uint8_t* bytes, size_t bytes_max, size_t byte_index,
                                        const quic_connection_id_t* odcid)
{
    size_t pseudo_index;
    ...
    {
        byte_index += picoquic_aead_encrypt_generic(bytes + byte_index, bytes + byte_index, 0, 0, pseudo_packet,
...
int picoquic_verify_retry_protection(void* integrity_aead, uint8_t* bytes, size_t* length, size_t byte_index,
                                     const quic_connection_id_t* odcid)
{
    ...
    if (byte_index + checksum_length < *length)
    {
        *length -= checksum_length;
        if ((pseudo_index = picoquic_format_retry_protection_pseudo_packet(pseudo_packet, bytes, *length, odcid)) > 0 &&
            picoquic_aead_decrypt_generic(decoded, bytes + *length, checksum_length, 0, pseudo_packet, pseudo_index,
                                          integrity_aead) == 0)
        {
            ret = 0;
        }
