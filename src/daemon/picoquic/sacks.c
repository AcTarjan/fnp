...
{
    fnp_free(picoquic_sack_node_value(node));
}
...
{
    int ret = 0;
    picoquic_sack_item_t* sack_new = (picoquic_sack_item_t*)fnp_zmalloc(sizeof(picoquic_sack_item_t));
    if (sack_new == NULL)
    {
        ret = -1;
    }
 ...
 */
picoquic_ack_context_t* picoquic_ack_ctx_from_cnx_context(quic_cnx_t* cnx,
 ...
 */
picoquic_sack_list_t* picoquic_sack_list_from_cnx_context(quic_cnx_t* cnx,
                                                          picoquic_packet_context_enum pc,
                                                          picoquic_local_cnxid_t* l_cid)
{
 ...
 */
picoquic_sack_item_t* picoquic_sack_find_range_below_number(picoquic_sack_list_t* sack_list,
 ...
 */
int picoquic_is_pn_already_received(quic_cnx_t* cnx,
...
// 接收方记录接收到的包序列号，更新path的ack_context
int picoquic_record_pn_received(quic_cnx_t* cnx,
                                picoquic_packet_context_enum pc,
                                picoquic_local_cnxid_t* l_cid,
...
{
    int cumul_sent = 0;
    int first_sack_count = (first_sack == NULL)
