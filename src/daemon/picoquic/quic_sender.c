...
int picoquic_mark_datagram_ready(quic_cnx_t* cnx, int is_ready)
{
    int ret = 0;
...
int picoquic_mark_datagram_ready_path(quic_cnx_t* cnx, uint64_t unique_path_id, int is_path_ready)
{
    int ret = 0;
...
void picoquic_set_datagram_priority(quic_cnx_t* cnx, uint8_t datagram_priority)
{
    cnx->datagram_priority = datagram_priority;
}

void picoquic_set_default_priority(quic_context_t* quic, uint8_t default_stream_priority)
{
    quic->default_stream_priority = default_stream_priority;
}

int picoquic_set_stream_priority(quic_cnx_t* cnx, uint64_t stream_id, uint8_t stream_priority)
{
    int ret = 0;
    quic_stream_t* stream = quic_find_or_create_local_stream(cnx, stream_id);
...
int picoquic_mark_high_priority_stream(quic_cnx_t* cnx, uint64_t stream_id, int is_high_priority)
{
    int ret;
...
int picoquic_open_flow_control(quic_cnx_t* cnx, uint64_t stream_id, uint64_t expected_data_size)
{
    int ret = 0;
    uint8_t buffer[512];
    size_t length = 0;
    ...
    if (cnx->cnx_state == picoquic_state_ready && cnx->quic->max_data_limit == 0)
    {
        ...
        else
        {
            ...
            {
                uint8_t* bytes_next = picoquic_format_max_stream_data_frame(
                    cnx, stream, buffer + consumed, bytes_max, &more_data, &is_pure_ack, max_required);
...
uint64_t picoquic_get_next_local_stream_id(quic_cnx_t* cnx, int is_unidir)
{
...
quic_packet_t* quic_create_packet(int size)
{
    ...
    // 考虑将其它字段初始化为0
    quic_packet_t* packet = (quic_packet_t*)rte_mbuf_to_priv(mbuf);
    packet->mbuf = mbuf;
    packet->buf_size = size;
    packet->bytes = rte_pktmbuf_mtod(mbuf, u8*);
    packet->length = 0;
    return packet;
}
    ...
    size_t* pn_length)
{
    ...
    else
    {
        ...
        (cnx->client_mode && (packet_type == picoquic_packet_initial ||
            ...
            ? &cnx->initial_cnxid
            : &path_x->first_tuple->p_remote_cnxid->cnx_id;
        quic_connection_id_t* srce_cnx_id = &path_x->first_tuple->p_local_cnxid->cnx_id;
        uint32_t version = ((cnx->cnx_state == picoquic_state_client_init || cnx->cnx_state ==
                               picoquic_state_client_init_sent) && packet_type == picoquic_packet_initial)
 ...
 */
int picoquic_get_checksum_length(quic_cnx_t* cnx, picoquic_epoch_enum epoch)
{
    size_t ret = 16;
...
size_t picoquic_protect_packet(quic_cnx_t* cnx,
                               ...
                               picoquic_path_t* path_x, picoquic_tuple_t* tuple, uint64_t current_time)
{
    ...
    h_length = picoquic_create_packet_header(cnx, ptype,
                                             sequence_number, path_x, tuple, header_length, send_buffer, &pn_offset,
                                             &pn_length);
    ...
    length = picoquic_pad_to_target_length(bytes, length, pn_sample_end - aead_checksum_length);
    // discount aead checksum length added later
    ...
    if (cnx->is_multipath_enabled && ptype == picoquic_packet_1rtt_protected)
    {
        send_length = picoquic_aead_encrypt_mp(send_buffer + /* header_length */ h_length,
                                               ...
                                               sequence_number, send_buffer, /* header_length */ h_length,
                                               aead_context);
    }
    else
    {
...
// 从发送队列中删除重传包, pending队列是发送队列, 并添加到retransmitted_oldest已重传队列中，
quic_packet_t* picoquic_dequeue_retransmit_packet(quic_cnx_t* cnx,
                                                  picoquic_packet_context_t* pkt_ctx, quic_packet_t* p,
                                                  int should_free)
{
    size_t dequeued_length = p->checksum_overhead + p->length;
 ...
 */
void picoquic_insert_hole_in_send_sequence_if_needed(quic_cnx_t* cnx, picoquic_path_t* path_x,
        ...
        pkt_ctx->send_sequence >= pkt_ctx->next_sequence_hole)
    {
            ...
            !pkt_ctx->pending_last->is_ack_trap)
        {
            ...
            if (packet != NULL)
            {
                ...
                picoquic_enqueue_for_retransmit(cnx, path_x, packet, 0, current_time);
        ...
        pkt_ctx->next_sequence_hole = pkt_ctx->send_sequence + 3 + picoquic_public_uniform_random(
            ((uint64_t)cnx->quic->sequence_hole_pseudo_period) << cnx->nb_packet_holes_inserted);
    }
}
                                         ...
                                         uint8_t* send_buffer, picoquic_path_t* path_x, uint64_t current_time)
{
    ...
    case picoquic_packet_version_negotiation:
        ...
        break;
    case picoquic_packet_initial:
        send_length = picoquic_protect_packet(cnx, packet->ptype, packet->bytes, packet->sequence_number,
                                              packet->length, packet->header_length,
                                              send_buffer, packet->buf_size,
                                              cnx->crypto_context[picoquic_epoch_initial].aead_encrypt,
                                              cnx->crypto_context[picoquic_epoch_initial].pn_enc,
    ...
    case picoquic_packet_handshake:
        send_length = picoquic_protect_packet(cnx, packet->ptype, packet->bytes, packet->sequence_number,
                                              ...
                                              cnx->crypto_context[picoquic_epoch_handshake].pn_enc,
                                              path_x, NULL, current_time);
        break;
    case picoquic_packet_retry:
        send_length = picoquic_protect_packet(cnx, packet->ptype, packet->bytes, packet->sequence_number,
                                              packet->length, packet->header_length,
                                              send_buffer, packet->buf_size,
                                              cnx->crypto_context[picoquic_epoch_0rtt].aead_encrypt,
                                              cnx->crypto_context[picoquic_epoch_0rtt].pn_enc,
                                              path_x, NULL, current_time);
        break;
    case picoquic_packet_0rtt_protected:
        send_length = picoquic_protect_packet(cnx, packet->ptype, packet->bytes, packet->sequence_number,
    ...
    case picoquic_packet_1rtt_protected:
        ...
        send_length = picoquic_protect_packet(cnx, packet->ptype, packet->bytes, packet->sequence_number,
                                              ...
                                              cnx->crypto_context[picoquic_epoch_1rtt].pn_enc,
                                              path_x, NULL, current_time);
        break;
    default:
    ...
    if (send_length > 0)
    {
        picoquic_enqueue_for_retransmit(cnx, path_x, packet, send_length, current_time);
        path_x->last_sent_time = current_time;
        path_x->bytes_sent += send_length;
    }
...
{
    int backlog_empty = 1;
    quic_packet_t* p = pkt_ctx->pending_first;
        ...
        if (!p->is_ack_trap && !p->is_multipath_probe && !p->is_mtu_probe)
        {
            while (ret == 0 && byte_index < p->length)
            {
                ret = picoquic_skip_frame(&p->bytes[byte_index],
                                          p->length - p->header_length, &frame_length, &frame_is_pure_ack);
...
int picoquic_is_cnx_backlog_empty(quic_cnx_t* cnx)
{
    int backlog_empty = 1;
 ...
 */
// static int picoquic_preemptive_retransmit_packet(quic_packet_t* old_p,
//                                                  picoquic_cnx_t* cnx,
//                                                  uint8_t* new_bytes,
...
//                                                         frame_length, old_p->ptype, &frame_is_pure_ack,
//                                                         &do_not_detect_spurious, &is_preemptive_needed);
//             }
//
//             /* Prepare retransmission if needed */
//             if (ret == 0 && !frame_is_pure_ack)
//             {
//                 if (PICOQUIC_IN_RANGE(old_p->bytes[byte_index], picoquic_frame_type_stream_range_min,
...
static size_t picoquic_next_mtu_probe_length(quic_cnx_t* cnx, picoquic_path_t* path_x)
{
    size_t probe_length;
    ...
    if (path_x->send_mtu_max_tried == 0)
    {
        if (cnx->remote_parameters.max_packet_size > 0)
        {
            ...
            if (cnx->quic->mtu_max > 0 && (int)probe_length >
                cnx->quic->mtu_max - PICOQUIC_MTU_OVERHEAD(&path_x->first_tuple->peer_addr))
            {
                probe_length = cnx->quic->mtu_max - PICOQUIC_MTU_OVERHEAD(&path_x->first_tuple->peer_addr);
            }
        ...
        else if (cnx->quic->mtu_max > 0)
        {
            probe_length = cnx->quic->mtu_max -
                PICOQUIC_MTU_OVERHEAD(&path_x->first_tuple->peer_addr);
        }
        else
        {
...
picoquic_pmtu_discovery_status_enum picoquic_is_mtu_probe_needed(quic_cnx_t* cnx, picoquic_path_t* path_x)
{
...
size_t picoquic_prepare_mtu_probe(quic_cnx_t* cnx,
                                  picoquic_path_t* path_x,
 ...
 */
int picoquic_prepare_packet_0rtt(quic_cnx_t* cnx, picoquic_path_t* path_x, quic_packet_t* packet,
                                 uint64_t current_time, uint8_t* send_buffer, size_t send_buffer_max,
                                 int* send_length, int padding_required, uint64_t* next_wake_time)
{
    int ret = 0;
    ...
    *send_length = picoquic_finalize_and_protect_packet(cnx, packet, send_buffer, path_x, current_time);
...
size_t picoquic_prepare_packet_old_context(quic_cnx_t* cnx, picoquic_packet_context_enum pc,
                                           picoquic_path_t* path_x, quic_packet_t* packet, size_t send_buffer_max,
                                           uint64_t current_time,
                                           uint64_t* next_wake_time, size_t* header_length)
{
    picoquic_epoch_enum epoch = (pc == picoquic_packet_context_initial)
                                    ? picoquic_epoch_initial
                                    : (pc == picoquic_packet_context_application)
                                    ? picoquic_epoch_0rtt
                                    : picoquic_epoch_handshake;
    size_t length = 0;
    ...
    if (cnx->crypto_context[epoch].aead_encrypt != NULL)
    {
        ...
        length = picoquic_retransmit_needed(cnx, pc, path_x, packet, current_time, next_wake_time);
        if (length > 0 &&
...
int picoquic_prepare_server_address_migration(quic_cnx_t* cnx)
{
    int ret = 0;
    ...
    if (cnx->remote_parameters.prefered_address.is_defined)
    {
        ...
        {
            fsockaddr_t dest_addr = {0};

            /* program a migration. */
            if (ipv4_received)
            {
                /* configure an IPv4 sockaddr */
                fsockaddr_t* d4 = &dest_addr;
                d4->family = FSOCKADDR_IPV4;
                d4->port = cnx->remote_parameters.prefered_address.ipv4Port;
                fnp_memcpy(&d4->ip, cnx->remote_parameters.prefered_address.ipv4Address, 4);
            }
             ...
             */
            if (fsockaddr_compare(&dest_addr,
                                  &cnx->path[0]->first_tuple->peer_addr) != 0 &&
                (cnx->quic->is_port_blocking_disabled || !quic_check_addr_blocked(&dest_addr)))
            {
                fsockaddr_t* local_addr = NULL;

                local_addr = &cnx->path[0]->first_tuple->local_addr;
                picoquic_probe_new_tuple(cnx, cnx->path[0], &dest_addr, local_addr, 0,
...
int picoquic_prepare_packet_client_init(quic_cnx_t* cnx, picoquic_path_t* path_x, quic_packet_t* packet,
                                        uint64_t current_time, uint8_t* send_buffer, int* send_length,
                                        uint64_t* next_wake_time, int* is_initial_sent)
{
    ...
    if (quic_stream_first_outcoming_data(&cnx->tls_stream[picoquic_epoch_initial]) == NULL)
    {
        if (cnx->crypto_context[picoquic_epoch_0rtt].aead_encrypt != NULL &&
            quic_stream_first_outcoming_data(&cnx->tls_stream[picoquic_epoch_0rtt]) != NULL)
        {
            epoch = picoquic_epoch_0rtt;
            pc = picoquic_packet_context_application;
            packet_type = picoquic_packet_0rtt_protected;
        }
        else if (cnx->crypto_context[picoquic_epoch_handshake].aead_encrypt != NULL && quic_stream_first_outcoming_data(
            &cnx->tls_stream[
                picoquic_epoch_0rtt]) == NULL)
        {
    ...
    int send_buffer_max = packet->buf_size;
        ...
        if (epoch > picoquic_epoch_initial)
        {
            if (cnx->crypto_context[picoquic_epoch_handshake].aead_encrypt != NULL)
            {
                if (cnx->ack_ctx[picoquic_packet_context_initial].act[0].ack_needed)
                {
                    ...
                    ack_time = cnx->ack_ctx[picoquic_packet_context_initial].act[0].time_oldest_unack_packet_received +
                        ack_delay;
        ...
        cnx->crypto_context[picoquic_epoch_0rtt].aead_encrypt != NULL)
    {
        length = picoquic_prepare_packet_old_context(cnx, picoquic_packet_context_application,
                                                     path_x, packet, send_buffer_max, current_time, next_wake_time,
                                                     &header_length);
    }
    ...
    if (length == 0)
    {
        ...
        if (ret == 0 && retransmit_possible &&
            (length = picoquic_retransmit_needed(cnx, pc, path_x, packet, current_time, next_wake_time)) > 0)
        {
            /* Check whether it makes sense to add an ACK at the end of the retransmission */
        ...
        else if (ret == 0)
        {
            ...
            else
            {
                ...
                if ((tls_ready == 0 || path_x->cwin <= path_x->bytes_in_transit || cnx->quic->cwin_max <= path_x->
                        bytes_in_transit)
                ...
                else
                {
                    ...
                    if (epoch != picoquic_epoch_0rtt && (cnx->ack_ctx[pc].act[0].ack_needed ||
                        (force_handshake_padding && picoquic_sack_list_last(&cnx->ack_ctx[pc].sack_list) !=
                            UINT64_MAX)))
                    {
                    ...
                    if (ret == 0 && path_x->cwin > path_x->bytes_in_transit && cnx->quic->cwin_max > path_x->
                        bytes_in_transit)
                    {
                        ...
                        if (tls_ready != 0)
                        {
                            ...
                            bytes_next = picoquic_format_crypto_hs_frame(&cnx->tls_stream[epoch],
                                                                         bytes_next, bytes_max, &more_data,
                                                                         &is_pure_ack);
                            length = bytes_next - bytes;
                        }
                        ...
                        if (packet_type == picoquic_packet_initial)
                        {
                            *is_initial_sent = 1;
                                ...
                                cnx->original_cnxid.id_len != 0)
                            {
                                ...
                                length = picoquic_pad_to_target_length(bytes, length,
                    ...
                    if (ret == 0 && tls_ready != 0 &&
                        quic_stream_first_outcoming_data(&cnx->tls_stream[epoch]) == NULL)
                    {
                        switch (cnx->cnx_state)
                        {
                        case picoquic_state_client_init:
                        ...
                        case picoquic_state_client_renegotiate:
                            ...
                            break;
                        case picoquic_state_client_almost_ready:
                            if (quic_stream_first_outcoming_data(&cnx->tls_stream[0]) == NULL &&
                                quic_stream_first_outcoming_data(&cnx->tls_stream[1]) == NULL &&
                                quic_stream_first_outcoming_data(&cnx->tls_stream[2]) == NULL)
                            {
                                cnx->cnx_state = picoquic_state_client_ready_start;
                                /* Signal the application, because data can now be sent. */
                                // if (cnx->callback_fn != NULL)
                                // {
                                //     if (cnx->callback_fn(cnx, 0, NULL, 0, picoquic_callback_almost_ready,
                                //                          cnx->callback_ctx, NULL) != 0)
                                //     {
                                //         picoquic_log_app_message(cnx, "Callback almost ready returns error 0x%x",
    ...
    else
    {
        ...
        if (ret == 0 && *is_initial_sent)
        {
            if (packet->ptype == picoquic_packet_initial)
            {
                if (length > 0 && cnx->crypto_context[1].aead_encrypt == NULL &&
                    (cnx->crypto_context[2].aead_encrypt == NULL || length + checksum_overhead +
                        PICOQUIC_MIN_SEGMENT_SIZE > send_buffer_max ||
                ...
                (cnx->crypto_context[3].aead_encrypt == NULL || length + checksum_overhead + PICOQUIC_MIN_SEGMENT_SIZE >
                    send_buffer_max))
            {
                length = picoquic_pad_to_target_length(bytes, length, send_buffer_max - checksum_overhead);
            }
        ...
        packet->length = length;
        *send_length = picoquic_finalize_and_protect_packet(cnx, packet, send_buffer,
                                                            path_x, current_time);
    }
...
int picoquic_prepare_packet_server_init(quic_cnx_t* cnx, picoquic_path_t* path_x, quic_packet_t* packet,
                                        ...
                                        int* is_initial_sent)
{
    ...
    //initial已经发送完成，进入handshake阶段
    if (cnx->crypto_context[picoquic_epoch_handshake].aead_encrypt != NULL &&
        quic_stream_first_outcoming_data(&cnx->tls_stream[picoquic_epoch_initial]) == NULL)
    {
        epoch = picoquic_epoch_handshake;
        pc = picoquic_packet_context_handshake;
        packet_type = picoquic_packet_handshake;
    ...
    if (length == 0)
    {
        checksum_overhead = picoquic_get_checksum_length(cnx, epoch);
        bytes_max = bytes + send_buffer_max - checksum_overhead;
        tls_ready = (quic_stream_first_outcoming_data(&cnx->tls_stream[epoch]) != NULL);
        length = picoquic_predict_packet_header_length(cnx, packet_type, &cnx->pkt_ctx[pc]);
        packet->ptype = packet_type;
        packet->header_length = length;
        header_length = length;
            ...
            || cnx->ack_ctx[pc].act[0].ack_needed)
        {
            ...
            *is_initial_sent |= (epoch == picoquic_epoch_initial && !is_pure_ack);
            ...
            if (ret == 0 && tls_ready != 0 && quic_stream_first_outcoming_data(&cnx->tls_stream[epoch]) == NULL)
            {
                if (epoch == picoquic_epoch_handshake && picoquic_tls_client_authentication_activated(cnx->quic) == 0)
                {
                    ...
                    // if (cnx->callback_fn != NULL)
                    // {
                    //     if (cnx->callback_fn(cnx, 0, NULL, 0, picoquic_callback_almost_ready, cnx->callback_ctx,
                    //                          NULL) != 0)
                    //     {
                    //         picoquic_log_app_message(cnx, "Callback almost ready returns error 0x%x",
                    //                                  PICOQUIC_TRANSPORT_INTERNAL_ERROR);
        ...
        else if ((length = picoquic_retransmit_needed(cnx, pc, path_x, packet, current_time, next_wake_time)) > 0)
        {
    ...
    if (ret == 0 && *is_initial_sent)
    {
        if (packet->ptype == picoquic_packet_initial)
        {
            // initial packet已经发送，但是还不能发送handshake阶段的数据包，则填充数据包
            if (length > 0 && cnx->crypto_context[1].aead_encrypt == NULL &&
                (cnx->crypto_context[2].aead_encrypt == NULL || length + checksum_overhead + PICOQUIC_MIN_SEGMENT_SIZE >
                    send_buffer_max ||
...
int picoquic_prepare_packet_closing(quic_cnx_t* cnx, picoquic_path_t* path_x, quic_packet_t* packet,
                                    uint64_t current_time, uint8_t* send_buffer, int* send_length,
                                    uint64_t* next_wake_time)
{
    int ret = 0;
            ...
            if (cnx->ack_ctx[pc].act[0].ack_needed)
            {
                ...
                if (cnx->local_error == 0)
                {
                    bytes_next = picoquic_format_application_close_frame(
                        cnx, bytes_next, bytes_max, &more_data, &is_pure_ack);
                }
                else
                {
                    bytes_next = picoquic_format_connection_close_frame(cnx, bytes_next, bytes_max, &more_data,
                                                                        &is_pure_ack);
                }
                length = bytes_next - bytes;
...
uint8_t* picoquic_format_new_local_id_as_needed(quic_cnx_t* cnx, uint8_t* bytes, uint8_t* bytes_max,
                                                uint64_t current_time, uint64_t* next_wake_time, int* more_data,
                                                int* is_pure_ack)
{
    int no_space_left = 0;
    ...
    while (local_cnxid_list != NULL && !no_space_left)
    {
        ...
        while (
            local_cnxid_list->nb_local_cnxid < ((int)(cnx->remote_parameters.active_connection_id_limit) +
            ...
            local_cnxid_list->nb_local_cnxid <= (PICOQUIC_NB_PATH_TARGET + local_cnxid_list->nb_local_cnxid_expired))
        {
            ...
            picoquic_local_cnxid_t* l_cid = picoquic_create_local_cnxid(cnx, local_cnxid_list->unique_path_id, NULL,
                                                                        current_time);
            ...
            {
                bytes = picoquic_format_new_connection_id_frame(cnx, local_cnxid_list, bytes, bytes_max, more_data,
                                                                is_pure_ack, l_cid);
...
void picoquic_false_start_transition(quic_cnx_t* cnx, uint64_t current_time)
{
    ...
    if (!cnx->client_mode && (cnx->quic->check_token || cnx->quic->provide_token))
    {
        ...
        size_t token_size;
        quic_connection_id_t n_cid = picoquic_null_connection_id;
...
{
    // 不能发送数据了
    if (cnx->data_sent > cnx->maxdata_remote)
        return NULL;
    ...
    {
        // 检查流是否就绪
        bool is_ready = (stream->sent_offset < stream->maxdata_remote &&
                quic_stream_first_outcoming_data(stream) != NULL) ||
            (stream->socket.request_reset && !stream->reset_sent) ||
            (stream->socket.request_stop_sending && !stream->stop_sending_sent);
        if (is_ready)
            break;
...
// 准备发送流数据（包括重传的流数据）
//
static uint8_t* quic_prepare_stream_and_datagrams(quic_cnx_t* cnx, picoquic_path_t* path_x,
                                                  ...
                                                  int* more_data, int* is_pure_ack, int* no_data_to_send, int* ret)
{
    ...
    u8* bytes_first = bytes_next;
    *ret = FNP_OK;

    while (bytes_next + 8 < bytes_max)
    {
        quic_stream_t* stream = quic_find_ready_stream(cnx);
        if (stream == NULL)
        {
            break;
        }

        if (stream->stream_priority >= max_priority_allowed)
        {
            break;
        }
        ...
        bytes_next = picoquic_format_stream_frame(cnx, stream, bytes_next, bytes_max, &more_stream_data, is_pure_ack);
        if (bytes_start == bytes_next)
        {
            break;
        }
    }

    *no_data_to_send = bytes_next == bytes_first;
 ...
 */
int picoquic_prepare_packet_almost_ready(quic_cnx_t* cnx, picoquic_path_t* path_x, quic_packet_t* packet,
                                         uint64_t current_time, uint8_t* send_buffer, int* send_length,
                                         uint64_t* next_wake_time,
                                         int* is_initial_sent)
{
    ...
    size_t send_buf_max = packet->buf_size;
    uint8_t* bytes_max = bytes + send_buf_max - checksum_overhead;
    ...
    if (!cnx->initial_validated &&
        (cnx->initial_data_sent + send_buf_max) > 3 * cnx->initial_data_received)
    {
        *send_length = 0;
        return 0;
    }
    ...
    if (path_x->first_tuple->p_local_cnxid != NULL)
    {
        ...
        {
            length = picoquic_prepare_packet_old_context(cnx, picoquic_packet_context_initial,
                                                         path_x, packet, send_buf_max, current_time,
                                                         next_wake_time, &header_length);
        }
        else
        {
        ...
        if (length == 0)
        {
            ...
            if (length == 0 && (*is_initial_sent == 1) && cnx->cnx_state == picoquic_state_server_false_start)
            {
                ...
                length = picoquic_predict_packet_header_length(cnx, packet->ptype,
                                                               &cnx->pkt_ctx[picoquic_packet_context_handshake]);
                header_length = length;
                packet->header_length = length;
            ...
            {
                checksum_overhead = picoquic_get_checksum_length(cnx, picoquic_epoch_handshake);
                bytes_max = bytes + send_buf_max - checksum_overhead;
            }
        }
        else
        {
        ...
        if (length > 0)
        {
            ...
            if (cnx->client_mode && *is_initial_sent && send_buf_max < length + checksum_overhead +
                PICOQUIC_MIN_SEGMENT_SIZE)
            {
                length = picoquic_pad_to_target_length(packet->bytes, length, send_buf_max - checksum_overhead);
            }
        }
    }
        ...
        if (cnx->cnx_state != picoquic_state_disconnected && path_x->first_tuple->challenge_verified != 0)
        {
            ...
            if (picoquic_is_sending_authorized_by_pacing(cnx, path_x, current_time, next_wake_time))
            {
                ...
                if (path_x->cwin < path_x->bytes_in_transit)
                {
                    ...
                    quic_congestion_notify(cnx, congestion_notification_cwin_blocked,
                                           ack_state.nb_bytes_acknowledged, current_time);
                }
                else
                {
                     ...
                     */
                    picoquic_pmtu_discovery_status_enum pmtu_discovery_needed = picoquic_is_mtu_probe_needed(
                        cnx, path_x);
                    ...
                    else
                    {
                        ...
                        if (length > header_length || pmtu_discovery_needed != picoquic_pmtu_discovery_required)
                        {
                            ...
                            if (ret == 0)
                            {
                                bytes_next = quic_prepare_stream_and_datagrams(cnx, path_x, bytes_next, bytes_max,
                            ...
                            if (length <= header_length)
                            {
                                ...
                                bytes_next = picoquic_format_blocked_frames(
                                    cnx, &bytes[length], bytes_max, &more_data, &is_pure_ack);
                                length = bytes_next - bytes;
                            }
                        ...
                        {
                            //TODO: 目前这个条件不会成立
                            if (send_buf_max > path_x->send_mtu)
                            {
                                /* Since there is no data to send, this is an opportunity to send an MTU probe */
                                length = picoquic_prepare_mtu_probe(cnx, path_x, header_length, checksum_overhead,
                                                                    bytes, send_buf_max);
                                packet->length = length;
                                packet->send_path = path_x;
    ...
    if (ret == 0 && length > header_length)
    {
        ...
        if ((*is_initial_sent && (packet->ptype != picoquic_packet_initial || length + checksum_overhead +
                PICOQUIC_MIN_SEGMENT_SIZE > send_buf_max || cnx->quic->dont_coalesce_init)) ||
            (is_challenge_padding_needed && length < PICOQUIC_ENFORCED_INITIAL_MTU))
        {
            length = picoquic_pad_to_target_length(bytes, length, (uint32_t)(send_buf_max - checksum_overhead));
        }
        else
        {
            length = picoquic_pad_to_policy(cnx, bytes, length, (uint32_t)(send_buf_max - checksum_overhead));
        }
    }
...
int picoquic_prepare_packet_ready(quic_cnx_t* cnx, picoquic_path_t* path_x, quic_packet_t* packet,
                                  ...
                                  uint64_t* next_wake_time, int* is_initial_sent)
{
    ...
    size_t length = 0;
    size_t checksum_overhead = picoquic_get_checksum_length(cnx, picoquic_epoch_1rtt);
    size_t send_buffer_max = packet->buf_size;
    uint8_t* bytes = packet->bytes;
    uint8_t* bytes_max = bytes + send_buffer_max - checksum_overhead;
    uint8_t* bytes_next;
    int more_data = 0;
    ...
    int is_nominal_ack_path = (cnx->is_multipath_enabled)
                                  ? (path_x->is_nominal_ack_path || cnx->nb_paths == 1)
                                  : path_x == cnx->path[0];
    ...
    else
    {
        ...
        else if (cnx->cnx_state != picoquic_state_disconnected && path_x->first_tuple->challenge_verified != 0)
        {
            ...
            if (picoquic_is_sending_authorized_by_pacing(cnx, path_x, current_time, next_wake_time))
            {
                ...
                if (ret == 0)
                {
                    if (cnx->quic->max_data_limit != 0)
                    {
                        ...
                        {
                            uint64_t max_data_increase = cnx->data_received + cnx->quic->max_data_limit - cnx->
                                maxdata_local;
                            bytes_next = picoquic_format_max_data_frame(cnx, bytes_next, bytes_max, &more_data,
                                                                        &is_pure_ack,
                                                                        max_data_increase);
                        }
                    }
                    ...
                    && !path_x->is_pto_required)
                {
                                                         ...
                                                         cnx->quic->packet_train_mode, cnx->quic))
                    {
                        bytes_next = quic_prepare_stream_and_datagrams(cnx, path_x, bytes_next, bytes_max,
                                                                       ...
                                                                       &more_data, &is_pure_ack, &no_data_to_send,
                                                                       &ret);
                    }
                    ...
                    else
                    {
                        ...
                        path_x->last_cwin_blocked_time = current_time;
                        picoquic_per_ack_state_t ack_state = {0};
                        quic_congestion_notify(cnx, congestion_notification_cwin_blocked, 0, current_time);
                    }
                }
                else
                {
                        ...
                        send_buffer_max <= path_x->send_mtu)
                    {
                        ...
                        {
                            bytes_next = quic_prepare_stream_and_datagrams(cnx, path_x, bytes_next, bytes_max,
                                                                           UINT64_MAX, current_time, &more_data,
                                                                           &is_pure_ack, &no_data_to_send, &ret);
                        }
                        ...
                        if (length <= header_length || is_pure_ack)
                        {
                            ...
                            path_x->delivered_limited_index = path_x->delivered;
                            /* Notify the peer if something is blocked */
                            bytes_next = picoquic_format_blocked_frames(cnx, &bytes[length], bytes_max, &more_data,
                                                                        &is_pure_ack);
                            length = bytes_next - bytes;
                        }
                                                 ...
                                                 cnx->quic->packet_train_mode, cnx->quic))
            {
                ...
                if ((bytes_next = quic_prepare_stream_and_datagrams(cnx, path_x, bytes_next, bytes_max,
                                                                    cnx->priority_limit_for_bypass, current_time,
                                                                    &more_data, &is_pure_ack, &no_data_to_send,
                                                                    &ret)) != NULL)
                {
    ...
    if (cnx->cnx_state != picoquic_state_disconnected)
    {
        if (length > 0)
        {
            ...
            if (is_pure_ack && cnx->is_multipath_enabled && path_x->is_ack_lost && !path_x->is_ack_expected)
            {
...
static int picoquic_check_idle_timer(quic_cnx_t* cnx, uint64_t* next_wake_time, uint64_t current_time)
{
    int ret = 0;
...
int picoquic_prepare_segment(quic_cnx_t* cnx, picoquic_path_t* path_x, quic_packet_t* packet,
                             uint64_t current_time, uint8_t* send_buffer, int* send_length,
                             uint64_t* next_wake_time, int* is_initial_sent)
{
    int ret = 0;
    ...
    case picoquic_state_client_handshake_start:
    case picoquic_state_client_almost_ready:
        ret = picoquic_prepare_packet_client_init(cnx, path_x, packet, current_time, send_buffer,
                                                  send_length, next_wake_time, is_initial_sent);
        break;
    case picoquic_state_server_almost_ready:
    case picoquic_state_server_init:
    case picoquic_state_server_handshake:
        ret = picoquic_prepare_packet_server_init(cnx, path_x, packet, current_time, send_buffer,
                                                  send_length, next_wake_time, is_initial_sent);
        break;
    case picoquic_state_server_false_start:
            ...
            cnx->crypto_context[3].aead_decrypt != NULL)
        {
            ...
            return picoquic_prepare_packet_ready(cnx, path_x, packet, current_time, send_buffer,
                                                 send_length, next_wake_time, is_initial_sent);
        }
    /* Else, just fall through to almost ready behavior.
     */
    case picoquic_state_client_ready_start:
        ret = picoquic_prepare_packet_almost_ready(cnx, path_x, packet, current_time, send_buffer,
                                                   send_length, next_wake_time, is_initial_sent);
        break;
    case picoquic_state_ready:
        ret = picoquic_prepare_packet_ready(cnx, path_x, packet, current_time, send_buffer,
                                            send_length, next_wake_time, is_initial_sent);
        break;
    case picoquic_state_handshake_failure:
    case picoquic_state_handshake_failure_resend:
    ...
    case picoquic_state_closing:
    case picoquic_state_draining:
        ret = picoquic_prepare_packet_closing(cnx, path_x, packet, current_time, send_buffer,
                                              send_length, next_wake_time);
        break;
    case picoquic_state_disconnected:
        ret = PICOQUIC_ERROR_DISCONNECTED;
        break;
...
static void picoquic_set_path_addresses_from_tuple(picoquic_tuple_t* tuple,
                                                   fsockaddr_t* p_addr_to,
                                                   fsockaddr_t* p_addr_from)
{
    if (p_addr_to != NULL)
    {
        fsockaddr_copy(p_addr_to, &tuple->peer_addr);
    }

    if (p_addr_from != NULL)
    {
        fsockaddr_copy(p_addr_from, &tuple->local_addr);
    }
}

/* manage the CC timer, if any */
static int picoquic_check_cc_feedback_timer(quic_cnx_t* cnx, uint64_t* next_wake_time, uint64_t current_time)
{
    int ret = 0;

    if (cnx->is_lost_feedback_notification_required)
    {
        for (int i = 0; i < cnx->nb_paths; i++)
        {
            ...
            if (!path_x->is_lost_feedback_notified)
            {
                ...
                if (pkt_ctx->pending_first != NULL)
                {
                    uint64_t delta_sent = (pkt_ctx->pending_first->send_time <= path_x->last_time_acked_data_frame_sent)
                                              ? 0
                                              : (pkt_ctx->pending_first->send_time - path_x->
                                                  last_time_acked_data_frame_sent);
                    uint64_t lost_feedback_time = pkt_ctx->highest_acknowledged_time + delta_sent + 2 * cnx->
                        ack_frequency_delay_local;

                    if (lost_feedback_time <= current_time)
                    {
                        path_x->is_lost_feedback_notified = 1;
                        quic_congestion_notify(cnx, congestion_notification_lost_feedback,
                                               0, current_time);
                    }
...
{
    int ret = 0;
    int send_buffer_max = PICOQUIC_MAX_PACKET_SIZE;
    quic_packet_t* packet = NULL;
    ...
    *send_length = 0;
    ...
    picoquic_set_path_addresses_from_tuple(tuple, remote, local);

    initial_next_time = next_wake_time;

    // udp数据包包含多个quic packet
    ...
    /* 一个udp mbuf包含多个quic packet */
    while (ret == 0)
    {
        // 超过最大包大小，退出
        if (buf_data_len + PICOQUIC_MIN_SEGMENT_SIZE > path_x->send_mtu)
        {
            break;
        }
        /* Create the segments that fit in the new packet */
        int avail_buf_size = send_buffer_max - buf_data_len;
        avail_buf_size = avail_buf_size > path_x->send_mtu ? path_x->send_mtu : avail_buf_size;

        packet = quic_create_packet(avail_buf_size);
        if (packet == NULL)
        {
        ...
        {
            ret = picoquic_prepare_path_control_packet(cnx, path_x, tuple, packet, current_time, send_buffer,
                                                       &segment_length, &next_wake_time);
        }
        else
        {
            ret = picoquic_prepare_segment(cnx, path_x, packet, current_time, send_buffer + buf_data_len,
                                           &segment_length, &next_wake_time, &is_initial_sent);
        }

        if (ret != FNP_OK)
        {
            picoquic_recycle_packet(packet);
            packet = NULL;
    ...
    if (buf_data_len > 0)
    {
        if (buf_data_len > cnx->max_mtu_sent)
        {
            cnx->max_mtu_sent = buf_data_len;
        }
        cnx->nb_packets_sent++;
    }

    /* Update the wake up time for the connection */
    if (buf_data_len > 0 || cnx->cnx_state == picoquic_state_disconnected)
    {
        next_wake_time = current_time;
        SET_LAST_WAKE(cnx->quic, PICOQUIC_SENDER);
    }
...
int picoquic_close(quic_cnx_t* cnx, uint64_t application_reason_code)
{
    int ret = 0;
...
void picoquic_close_immediate(quic_cnx_t* cnx)
{
    if (cnx->cnx_state < picoquic_state_draining)
    {
 ...
 * will send a stateless packet if one is queued,
 * or ask the first connection in the wake list to prepare a packet
 */
struct rte_mbuf* quic_prepare_packet_tx(quic_context_t* quic, u64 current_time)
{
    // 先发送无状态数据包
    ...
    quic_cnx_t* cnx = picoquic_get_earliest_cnx_to_wake(quic, current_time);
    if (cnx == NULL)
    {
        return NULL;
    }
    ...
    int ret = picoquic_prepare_packet(cnx, current_time, &info->local, &info->remote, buf, &data_len);
    if (ret != FNP_OK)
    {
        if (ret == PICOQUIC_ERROR_DISCONNECTED)
        {
            picoquic_log_app_message(
                cnx, "Closed. Retrans= %d, spurious= %d, max sp gap = %d, max sp delay = %d, dg-coal: %f",
...
void quic_send_udp_mbuf(quic_context_t* quic)
{
    fsocket_t* udp_socket = quic->udp_socket;
    ...
    {
        // 发送数据
        struct rte_mbuf* m = quic_prepare_packet_tx(quic, current_time);
        if (m == NULL)
        {
            break;
        }
