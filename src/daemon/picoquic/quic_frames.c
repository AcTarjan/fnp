...
int picoquic_process_ack_of_max_data_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                           size_t bytes_max, size_t* consumed);
int picoquic_process_ack_of_max_stream_data_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                  size_t bytes_size, size_t* consumed);
int picoquic_process_ack_of_max_streams_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                              size_t bytes_size, size_t* consumed);
int picoquic_check_max_streams_frame_needs_repeat(quic_cnx_t* cnx, const uint8_t* bytes,
                                                  const uint8_t* p_last_byte, int* no_need_to_repeat);
int picoquic_path_available_or_backup_frame_need_repeat(quic_cnx_t* cnx, const uint8_t* bytes,
                                                        const uint8_t* bytes_max, int* no_need_to_repeat);
int picoquic_max_path_id_frame_needs_repeat(quic_cnx_t* cnx, const uint8_t* bytes,
                                            const uint8_t* bytes_max, int* no_need_to_repeat);
int picoquic_process_ack_of_max_path_id_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                              size_t bytes_max, size_t* consumed);
int picoquic_paths_blocked_frame_needs_repeat(quic_cnx_t* cnx, const uint8_t* bytes,
                                              const uint8_t* bytes_max, int* no_need_to_repeat);
int picoquic_process_ack_of_paths_blocked_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                size_t bytes_max, size_t* consumed);
int picoquic_path_cid_blocked_frame_needs_repeat(quic_cnx_t* cnx, const uint8_t* bytes,
                                                 const uint8_t* bytes_max, int* no_need_to_repeat);
int picoquic_process_ack_of_path_cid_blocked_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                   size_t bytes_max, size_t* consumed);
int picoquic_process_ack_of_observed_address_frame(quic_cnx_t* cnx, picoquic_path_t* path_x, const uint8_t* bytes,
...
int picoquic_is_stream_closed(quic_stream_t* stream, int client_mode)
{
    ...
    {
        is_closed = stream->socket.request_close && (stream->fin_sent || (stream->reset_sent)) &&
            (stream->socket.receive_fin || stream->socket.receive_reset);
    }
    ...
    {
        // 远端单向流
        is_closed = stream->socket.request_close && (stream->socket.receive_fin || stream->socket.receive_reset);
    }

    return is_closed;
}

int picoquic_is_stream_acked(quic_stream_t* stream)
{
    int is_acked = 0;
...
{
    quic_stream_t* stream = picoquic_first_stream(cnx);
...
int picoquic_flow_control_check_stream_offset(quic_cnx_t* cnx, quic_stream_t* stream,
...
uint8_t* picoquic_format_stream_reset_frame(quic_cnx_t* cnx, quic_stream_t* stream,
                                            uint8_t* bytes, uint8_t* bytes_max, int* more_data, int* is_pure_ack)
{
    ...
    if ((bytes = picoquic_frames_uint8_encode(bytes, bytes_max, picoquic_frame_type_reset_stream)) != NULL &&
        ...
        (bytes = picoquic_frames_varint_encode(bytes, bytes_max, stream->socket.local_error)) != NULL &&
...
const uint8_t* picoquic_decode_stream_reset_frame(quic_cnx_t* cnx, const uint8_t* bytes, const uint8_t* bytes_max)
{
    ...
    else if ((stream = quic_find_or_create_remote_stream(cnx, stream_id)) == NULL)
    {
    ...
    else if ((stream->fin_received || stream->socket.receive_reset) && final_offset != stream->fin_offset)
    {
    ...
    else if (!stream->socket.receive_reset)
    {
        stream->socket.receive_reset = 1;
        stream->socket.remote_error = error_code_64;
...
int picoquic_process_ack_of_reset_stream_frame(quic_cnx_t* cnx, const uint8_t* bytes, size_t bytes_size,
                                               size_t* consumed)
{
    int ret = 0;
...
int picoquic_check_reset_stream_needs_repeat(quic_cnx_t* cnx, const uint8_t* bytes, size_t bytes_size,
                                                 ...
                                                 int* more_data, int* is_pure_ack, picoquic_local_cnxid_t* l_cid)
{
    ...
    {
        if ((bytes = picoquic_frames_varint_encode(bytes, bytes_max, picoquic_frame_type_new_connection_id)) == NULL ||
            ...
            (bytes = picoquic_frames_varint_encode(bytes, bytes_max, local_cnxid_list->local_cnxid_retire_before)) ==
            NULL ||
...
const uint8_t* picoquic_parse_new_connection_id_frame(const uint8_t* bytes, const uint8_t* bytes_max,
                                                      ...
                                                      uint64_t* sequence, uint64_t* retire_before, uint8_t* cid_length,
...
const uint8_t* picoquic_decode_new_connection_id_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                       const uint8_t* bytes_max, uint64_t current_time, int is_mp)
{
    ...
    bytes = picoquic_parse_new_connection_id_frame(bytes, bytes_max, is_mp, &unique_path_id, &sequence,
                                                   &retire_before, &cid_length, &cnxid_bytes, &secret_bytes);
    ...
    else
    {
        ...
        else
        {
            uint64_t transport_error = picoquic_add_remote_cnxid_to_stash(cnx, remote_cnxid_stash, retire_before,
                                                                          sequence, cid_length, cnxid_bytes,
                                                                          secret_bytes, NULL);
            ...
            if (transport_error != 0)
            {
                picoquic_connection_error(cnx, transport_error, picoquic_frame_type_new_connection_id);
                bytes = NULL;
            }
        }
    }
...
int picoquic_process_ack_of_new_cid_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                          size_t bytes_max, int is_mp, size_t* consumed)
{
    ...
    const uint8_t* bytes_next = picoquic_parse_new_connection_id_frame(bytes, bytes + bytes_max, is_mp, &unique_path_id,
                                                                       &sequence, &retire_before, &cid_length,
                                                                       &cnxid_bytes, &secret_bytes);
...
int picoquic_check_new_cid_needs_repeat(quic_cnx_t* cnx, const uint8_t* bytes,
...
uint8_t* picoquic_format_retire_connection_id_frame(uint8_t* bytes, uint8_t* bytes_max, int* more_data,
                                                    int* is_pure_ack,
                                                    int is_mp, uint64_t unique_path_id, uint64_t sequence)
{
    uint8_t* bytes0 = bytes;

    if ((bytes = picoquic_frames_varint_encode(bytes, bytes_max, picoquic_frame_type_retire_connection_id)) == NULL ||
...
int picoquic_queue_retire_connection_id_frame(quic_cnx_t* cnx, uint64_t unique_path_id, uint64_t sequence)
{
    ...
    uint8_t* bytes_next = picoquic_format_retire_connection_id_frame(frame_buffer, frame_buffer + sizeof(frame_buffer),
                                                                     ...
                                                                     cnx->is_multipath_enabled, unique_path_id,
                                                                     sequence);
...
int picoquic_queue_new_token_frame(quic_cnx_t* cnx, uint8_t* token, size_t token_length)
{
    ...
    int is_pure_ack = 1;
    uint8_t frame_buffer[258];
    uint8_t* bytes = picoquic_format_new_token_frame(frame_buffer, frame_buffer + sizeof(frame_buffer), &more_data,
                                                     &is_pure_ack, token, token_length);
...
const uint8_t* picoquic_decode_new_token_frame(quic_cnx_t* cnx, const uint8_t* bytes, const uint8_t* bytes_max,
                                               uint64_t current_time, fsockaddr_t* addr_to)
{
...
const uint8_t* picoquic_decode_retire_connection_id_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                          ...
                                                          picoquic_path_t* path_x, int is_mp)
{
    ...
    if ((bytes = picoquic_frames_varint_skip(bytes, bytes_max)) == NULL ||
        (bytes = picoquic_parse_retire_connection_id_frame(bytes, bytes_max, &unique_path_id, &sequence, is_mp)) ==
        NULL)
    {
        picoquic_connection_error(cnx, PICOQUIC_TRANSPORT_FRAME_FORMAT_ERROR,
...
int picoquic_check_retire_connection_id_needs_repeat(quic_cnx_t* cnx, const uint8_t* bytes,
                                                     size_t bytes_size, int* no_need_to_repeat, int is_mp)
{
    ...
    const uint8_t* bytes_next = (bytes_first == NULL)
                                    ? NULL
                                    : picoquic_parse_retire_connection_id_frame(
                                        bytes_first, bytes + bytes_size, &unique_path_id, &sequence, is_mp);
    *no_need_to_repeat = 1;
...
int picoquic_process_ack_of_retire_connection_id_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                       size_t bytes_size, size_t* consumed, int is_mp)
{
    int ret = 0;
    uint64_t sequence = 0;
    uint64_t unique_path_id = 0;
    const uint8_t* bytes_next = picoquic_parse_retire_connection_id_frame(
        bytes + 1, bytes + bytes_size, &unique_path_id, &sequence, is_mp);
...
uint8_t* picoquic_format_stop_sending_frame(quic_stream_t* stream, uint8_t* bytes, uint8_t* bytes_max, int* more_data,
                                            int* is_pure_ack)
{
    if (stream->fin_received || stream->socket.receive_reset)
    {
    ...
    if ((bytes = picoquic_frames_uint8_encode(bytes, bytes_max, picoquic_frame_type_stop_sending)) != NULL &&
        ...
        (bytes = picoquic_frames_varint_encode(bytes, bytes_max, stream->socket.local_stop_error)) != NULL
    )
    {
        *is_pure_ack = 0;
        stream->stop_sending_sent = 1;
...
const uint8_t* picoquic_decode_stop_sending_frame(quic_cnx_t* cnx, const uint8_t* bytes, const uint8_t* bytes_max)
{
    uint64_t stream_id = 0;
    ...
    else if (!stream->socket.receive_stop_sending && !stream->socket.request_reset && !stream->fin_sent)
    {
        stream->socket.receive_stop_sending = 1;
        stream->socket.remote_stop_error = error_code;
    }
    else
    {
        /* The stream is already finished. Should just ignore the frame */
...
int picoquic_check_stop_sending_needs_repeat(quic_cnx_t* cnx, const uint8_t* bytes, size_t bytes_size,
                                             int* no_need_to_repeat)
{
    uint64_t stream_id = 0;
    uint64_t error_code = 0;
    ...
    *no_need_to_repeat = 0;
    ...
    else if (stream->fin_received || stream->socket.receive_reset)
    {
...
{
    return PICOQUIC_BITS_CLEAR_IN_RANGE(bytes[0], picoquic_frame_type_stream_range_min,
                                        picoquic_frame_type_stream_range_max, 0x02);
}
                                 ...
                                 size_t* consumed)
{
    ...
    *fin = bytes[0] & 1;
    ...
    else
    {
        ...
        {
            DBG_PRINTF(
                "stream data past the end of the packet: first_byte=0x%02x, data_length=%" PRIst ", max_bytes=%" PRIst,
...
bool quic_stream_check_incoming_data(quic_stream_t* stream, quic_stream_data_t* stream_data)
{
    u64 offset = stream_data->offset;
    u64 offset_end = stream_data->offset + stream_data->length;

    if (offset_end >= stream->consumed_offset)
    {
        if (offset <= stream->consumed_offset)
        {
            int covered_len = stream->consumed_offset - offset; //去掉重复的数据
            stream_data->bytes += covered_len;
            stream_data->length -= covered_len;
            ...
            stream->consumed_offset += stream_data->length; // 更新消费偏移量
            return true;
        }
    }
    else
...
static int picoquic_stream_network_input(quic_cnx_t* cnx, uint64_t stream_id, uint64_t offset, int fin,
                                         const uint8_t* bytes, size_t length,
                                         quic_packet_t* packet, uint64_t current_time)
{
    int ret = 0;
    ...
    if ((stream = quic_find_or_create_remote_stream(cnx, stream_id)) == NULL)
    {
    ...
    {
        if (fin != 0 ? stream->fin_offset != offset_end : offset_end > stream->fin_offset)
        {
    ...
    else
    {
        ...
        if (offset_end > stream->fin_offset)
        {
            ret = picoquic_flow_control_check_stream_offset(cnx, stream, offset_end);
        }
    }
    ...
    if (ret == 0)
    {
        ...
        if (offset_end >= stream->consumed_offset)
        {
            int data_offset = bytes - packet->bytes;
            quic_stream_data_t* stream_data = quic_create_stream_data_from_packet(packet, data_offset, length);
            if (stream_data == NULL)
            {
                return -1;
            }
            stream_data->stream_id = stream_id;
            stream_data->offset = offset;
            stream_data->fin = fin;
            stream_data->enqueue_app = 0; // 标记未入队

            if (quic_stream_check_incoming_data(stream, stream_data))
            {
                // 顺序的数据，成功入队，继续检查
                while (1)
                {
                    quic_stream_data_t* first = quic_stream_first_incoming_data(stream);
                    if (first == NULL)
                        break;
                    // 成功传递到应用层
                    if (quic_stream_check_incoming_data(stream, first))
                    {
                        // 从乱序队列中删除
                        quic_stream_dequeue_incoming_data(stream, first);
                        continue;
                    }
                    break;
                }
            ...
            {
                // 乱序的数据
                quic_stream_enqueue_incoming_data(stream, stream_data);
            }
        }
        else
        {
                                            ...
                                            uint64_t current_time)
{
    ...
    else
    {
        ...
        * follows. It is used to manage the queue of stream chunks awaiting delivery.
         */
        bytes += consumed; //去掉帧头
        if (picoquic_stream_network_input(cnx, stream_id, offset, fin,
                                          bytes, data_length, packet,
                                          current_time) != 0)
        {
            bytes = NULL;
        }
...
uint8_t* picoquic_format_data_blocked_frame(quic_cnx_t* cnx, uint8_t* bytes,
...
uint8_t* picoquic_format_stream_blocked_frame(quic_cnx_t* cnx, uint8_t* bytes,
                                              uint8_t* bytes_max, int* more_data, int* is_pure_ack,
                                              quic_stream_t* stream)
{
    uint8_t* bytes0 = bytes;
...
uint8_t* picoquic_format_one_blocked_frame(quic_cnx_t* cnx, uint8_t* bytes,
                                           uint8_t* bytes_max, int* more_data, int* is_pure_ack,
                                           quic_stream_t* stream)
{
    if (quic_stream_first_outcoming_data(stream) != NULL)
    {
        ...
        if (IS_CLIENT_STREAM_ID(stream->stream_id) == cnx->client_mode &&
            stream->stream_id > ((IS_BIDIR_STREAM_ID(stream->stream_id))
                                     ? cnx->max_stream_id_bidir_remote
                                     : cnx->max_stream_id_unidir_remote))
        {
            if (!(IS_BIDIR_STREAM_ID(stream->stream_id)
                      ? cnx->stream_blocked_bidir_sent
                      : cnx->stream_blocked_unidir_sent))
            {
...
{
    quic_stream_t* stream = picoquic_first_stream(cnx);
    quic_stream_t* hi_pri_stream = NULL;
...
uint8_t* picoquic_provide_stream_data_buffer(void* context, size_t length, int is_fin, int is_still_active)
{
    ...
    if (length <= data_ctx->allowed_space)
    {
        ...
        data_ctx->byte_index = picoquic_encode_length_of_stream_frame(data_ctx->bytes,
                                                                      data_ctx->byte_index, data_ctx->byte_space,
                                                                      length, &start_index);
...
uint8_t* picoquic_format_stream_frame(quic_cnx_t* cnx, quic_stream_t* stream, uint8_t* bytes, uint8_t* bytes_max,
                                      int* more_data, int* is_pure_ack)
{
    ...
    if (stream->socket.request_reset)
    {
        return picoquic_format_stream_reset_frame(cnx, stream, bytes, bytes_max, more_data, is_pure_ack);
    }

    if (stream->socket.request_stop_sending && !stream->stop_sending_sent)
    {
        return picoquic_format_stop_sending_frame(stream, bytes, bytes_max, more_data, is_pure_ack);
    }

    // 获取stream_data, fin会携带在stream_data中
    quic_stream_data_t* stream_data = quic_stream_first_outcoming_data(stream);
    if (stream_data == NULL)
    {
        /* Nothing to send */
        return bytes;
    ...
    // 必有数据
    if ((bytes = picoquic_format_stream_frame_header(bytes, bytes_max, stream->stream_id, stream_data->offset)) == NULL)
    {
        bytes = bytes0;
        *more_data = 1;
        return bytes;
    ...
    int length = stream_data->length <= allowed_space ? stream_data->length : allowed_space;
    if (length > 0 || stream_data->fin)
    {
        byte_index = picoquic_encode_length_of_stream_frame(bytes0, byte_index, byte_space, length, &start_index);
        fnp_memcpy(&bytes0[byte_index], stream_data->bytes, length);
        byte_index += length;
        ...
        {
            /* Remove the stream data node from the queue */
            if (stream_data->fin)
            {
                bytes0[start_index] |= 1;
                stream->fin_sent = 1;
                // picoquic_remove_output_stream(cnx, stream);      暂时还不能删除
                picoquic_update_max_stream_ID_local(cnx, stream);
            }
            quic_stream_dequeue_outcoming_data(stream, stream_data);
        }
...
int picoquic_is_tls_stream_ready(quic_cnx_t* cnx)
{
    ...
    {
        quic_stream_t* stream = &cnx->tls_stream[epoch];
                                               ...
                                               int epoch)
{
    ...
    else
    {
        ...
        {
            // 处理收到的数据
            u64 offset_end = offset + data_length;
            if (offset_end >= stream->consumed_offset) //无论乱序还是顺序都插入进去
            {
                int data_offset = data_bytes - packet->bytes;
                quic_stream_data_t* stream_data = quic_create_stream_data_from_packet(packet, data_offset, data_length);
                if (stream_data == NULL)
                {
                    printf("Error: Failed to create stream data from packet.\n");
                    return NULL;
                }
                stream_data->offset = offset;
                stream_data->fin = 0;
...
static quic_stream_t* picoquic_crypto_stream_from_ptype(quic_cnx_t* cnx, picoquic_packet_type_enum p_type)
{
...
int picoquic_process_ack_of_crypto_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                         size_t bytes_size, picoquic_packet_type_enum p_type, size_t* consumed)
{
    ...
    {
        quic_stream_t* stream = picoquic_crypto_stream_from_ptype(cnx, p_type);
...
int picoquic_check_crypto_frame_needs_repeat(quic_cnx_t* cnx, const uint8_t* bytes,
                                             size_t bytes_size, picoquic_packet_type_enum p_type,
                                             int* no_need_to_repeat)
{
    int ret = 0;
    uint64_t offset = 0;
                                         ...
                                         int* more_data, int* is_pure_ack)
{
    ...
    if ((stream_data = quic_stream_first_outcoming_data(stream)) != NULL)
    {
        ...
        if ((bytes = picoquic_frames_uint8_encode(bytes, bytes_max, picoquic_frame_type_crypto_hs)) != NULL &&
            (bytes = picoquic_frames_varint_encode(bytes, bytes_max, stream_data->offset)) != NULL)
        {
            /* As there is enough room, estimate the actual length, then encode the packet */
            size_t length = stream_data->length;
            uint8_t* bytes_l;
            ...
            else
            {
                ...
                {
                    fnp_memcpy(bytes, stream_data->bytes, length);
                    bytes += length;
...
// 判断是否发生了伪重传
quic_packet_t* picoquic_check_spurious_retransmission(quic_cnx_t* cnx,
                                                      picoquic_packet_context_enum pc,
                                                      picoquic_packet_context_t* pkt_ctx,
                                                      uint64_t start_of_range, uint64_t end_of_range,
                                                      uint64_t current_time, uint64_t time_stamp,
                                                      quic_packet_t* p, picoquic_packet_data_t* packet_data)
{
    // 从已重传的数据队列判断
    while (p != NULL && p->sequence_number >= start_of_range)
    {
        quic_packet_t* should_delete = NULL;

        // 旧的已重传的数据包被确认了，说明发生了伪重传
        if (p->sequence_number <= end_of_range)
        {
            ...
            if (old_path != NULL)
            {
                ...
                quic_congestion_notify(cnx, congestion_notification_spurious_repeat,
                                       0, current_time);
            }
...
// 删除已重传的数据包
void picoquic_dequeue_old_retransmitted_packets(quic_cnx_t* cnx, picoquic_packet_context_t* pkt_ctx)
{
    quic_packet_t* p = pkt_ctx->retransmitted_oldest;
...
void picoquic_estimate_path_bandwidth(quic_cnx_t* cnx, picoquic_path_t* path_x, uint64_t send_time,
                                      uint64_t delivered_prior, uint64_t delivered_time_prior,
...
void picoquic_estimate_max_path_bandwidth(quic_cnx_t* cnx, picoquic_path_t* path_x, uint64_t send_time,
...
static uint64_t picoquic_compute_packets_in_window(quic_cnx_t* cnx, uint64_t data_rate)
{
...
//根据当前连接的带宽（data_rate）和期望的包数量（nb_packets）
//计算出发送 ACK（确认包）之间应该间隔多少个数据包。
// 这个间隔用于动态调整 ACK 的发送频率，以便在高带宽或高吞吐量场景下减少 ACK 包的数量。
static uint64_t picoquic_compute_ack_gap(quic_cnx_t* cnx, uint64_t data_rate, uint64_t nb_packets)
{
    uint64_t ack_gap;
    ...
    else if (ack_gap > 32)
    {
        ...
        if (cnx->is_multipath_enabled ||
            algo->id == congestion_algo_new_reno ||
            algo->id == congestion_algo_fast
        )
        {
...
uint64_t picoquic_compute_ack_delay_max(quic_cnx_t* cnx, uint64_t rtt, uint64_t remote_min_ack_delay)
{
                                        ...
                                        uint64_t data_rate, uint64_t* ack_gap, uint64_t* ack_delay_max)
{
    ...
    *ack_gap = picoquic_compute_ack_gap(cnx, data_rate, nb_packets);
    ...
    if (2 * cnx->path[0]->smoothed_rtt > 3 * cnx->path[0]->rtt_min)
    {
        ...
        uint64_t return_data_rate = cnx->path[0]->bandwidth_estimate;
...
void process_decoded_packet_data(quic_cnx_t* cnx, picoquic_path_t* path_x,
                                 int epoch, uint64_t current_time, picoquic_packet_data_t* packet_data)
{
    for (int i = 0; i < packet_data->nb_path_ack; i++)
    {
        ...
        picoquic_estimate_path_bandwidth(cnx, packet_data->path_ack[i].acked_path,
                                         packet_data->path_ack[i].largest_sent_time,
                                         packet_data->path_ack[i].delivered_prior,
                                         packet_data->path_ack[i].delivered_time_prior,
                                         packet_data->path_ack[i].delivered_sent_prior,
                                         (packet_data->last_time_stamp_received == 0)
                                             ? current_time
                                             : packet_data->last_time_stamp_received,
                                         current_time, packet_data->path_ack[i].rs_is_path_limited);

        picoquic_estimate_max_path_bandwidth(cnx, packet_data->path_ack[i].acked_path,
                                             packet_data->path_ack[i].largest_sent_time,
                                             (packet_data->last_time_stamp_received == 0)
                                                 ? current_time
                                                 : packet_data->last_time_stamp_received,
        ...
        if (packet_data->path_ack[i].acked_path->rtt_sample > 0)
        {
            picoquic_per_ack_state_t ack_state = {0};
            ...
            if (cnx->cnx_state == picoquic_state_ready)
            {
                ack_state.nb_bytes_lost_since_packet_sent = path_x->total_bytes_lost - packet_data->path_ack[i].
                    lost_prior;
            }
            else
            {
            ...
            ack_state.nb_bytes_delivered_since_packet_sent = path_x->delivered - packet_data->path_ack[i].
                delivered_prior;
            ...
            quic_congestion_notify(cnx, congestion_notification_acknowledgement,
...
static quic_packet_t* picoquic_find_acked_packet(quic_cnx_t* cnx, picoquic_packet_context_t* pkt_ctx,
                                                 uint64_t largest, uint64_t current_time, int* is_new_ack)
{
    quic_packet_t* packet = pkt_ctx->pending_first;
    ...
    uint8_t* bytes, size_t bytes_max, size_t* consumed, int is_ecn)
{
    ...
    while (1)
    {
        ...
        if (range > 0)
        {
            previous_sack_item = picoquic_process_ack_of_ack_range(sack_list, previous_sack_item, largest + 1 - range,
                                                                   largest);
        }
...
static const uint8_t* picoquic_skip_ack_frame_maybe_ecn(const uint8_t* bytes, const uint8_t* bytes_max, int is_ecn,
                                                        int has_path);
...
int picoquic_process_ack_of_path_ack_frame(
    quic_cnx_t* cnx, uint8_t* bytes, size_t bytes_max, size_t* consumed, int is_ecn)
{
    int ret;
    uint64_t largest;
    uint64_t ack_delay;
    ...
    if (ret == 0)
    {
        ...
        {
            ret = picoquic_process_ack_of_ack_body(&ack_ctx->sack_list, largest, num_block, bytes, bytes_max, consumed,
                                                   is_ecn);
        }
    }
    ...
    *no_need_to_repeat = 0;
    ...
    if (PICOQUIC_IN_RANGE(bytes[0], picoquic_frame_type_stream_range_min, picoquic_frame_type_stream_range_max))
    {
        ...
        if (ret == 0)
        {
            ...
            else
            {
                ...
                {
                    /* Check whether the ack was already received */
                    *no_need_to_repeat = picoquic_check_sack_list(&stream->sack_list, offset,
                                                                  offset + data_length - ((fin) ? 0 : 1));
                }
    ...
    else
    {
        ...
        case picoquic_frame_type_max_stream_data:
            ...
            else if (stream->fin_received || stream->socket.receive_reset || stream->stop_sending_sent)
            {
        ...
        case picoquic_frame_type_stream_data_blocked:
            ...
            else if (stream->socket.request_fin || stream->socket.request_reset
                || stream->fin_sent || stream->reset_sent)
            {
                /* Stream stopped, no need to increase the window */
                *no_need_to_repeat = 1;
            }
...
int picoquic_process_ack_of_stream_frame(quic_cnx_t* cnx, uint8_t* bytes,
                                         size_t bytes_max, size_t* consumed)
{
    int ret;
    int fin;
                                    ...
                                    int is_spurious, uint64_t current_time)
{
    ...
    int offset = p->header_length;
    int length = p->length;

    while (ret == 0 && offset < length)
    {
        uint64_t ftype;
        u8* data = p->bytes + offset;
        size_t l_ftype = picoquic_varint_decode(data, length - offset, &ftype);
        if (l_ftype == 0)
        {
            break;
        }
        ...
        case picoquic_frame_type_ack:
            ret = picoquic_process_ack_of_ack_frame(&cnx->ack_ctx[p->pc].sack_list,
                                                    data, length - offset, &frame_length, 0);
            offset += frame_length;
            break;
        case picoquic_frame_type_ack_ecn:
            ret = picoquic_process_ack_of_ack_frame(&cnx->ack_ctx[p->pc].sack_list,
                                                    data, length - offset, &frame_length, 1);
            offset += frame_length;
            break;
        case picoquic_frame_type_handshake_done:
            ...
            break;
        case picoquic_frame_type_new_connection_id:
            ret = picoquic_process_ack_of_new_cid_frame(cnx, data, length - offset, 0,
                                                        &frame_length);
            offset += frame_length;
            break;
        case picoquic_frame_type_retire_connection_id:
            ret = picoquic_process_ack_of_retire_connection_id_frame(cnx, data, length - offset,
                                                                     &frame_length, 0);
            offset += frame_length;
            break;
        case picoquic_frame_type_crypto_hs:
            ret = picoquic_process_ack_of_crypto_frame(cnx, data, length - offset, p->ptype,
                                                       &frame_length);
            offset += frame_length;
            break;
        case picoquic_frame_type_new_token:
            ret = picoquic_skip_frame(data, length - offset, &frame_length, &frame_is_pure_ack);
            offset += frame_length;
            cnx->is_new_token_acked = 1;
            break;
        case picoquic_frame_type_max_data:
            ret = picoquic_process_ack_of_max_data_frame(cnx, data, length - offset, &frame_length);
            offset += frame_length;
            break;
        case picoquic_frame_type_max_stream_data:
            ret = picoquic_process_ack_of_max_stream_data_frame(cnx, data, length - offset,
                                                                &frame_length);
            offset += frame_length;
            break;
        case picoquic_frame_type_max_streams_bidir:
        case picoquic_frame_type_max_streams_unidir:
            ret = picoquic_process_ack_of_max_streams_frame(cnx, data, length - offset,
                                                            &frame_length);
            offset += frame_length;
            break;
        case picoquic_frame_type_reset_stream:
            ret = picoquic_process_ack_of_reset_stream_frame(cnx, data, length - offset, &frame_length);
            offset += frame_length;
            break;
        default:
            if (PICOQUIC_IN_RANGE(ftype, picoquic_frame_type_stream_range_min, picoquic_frame_type_stream_range_max))
            {
                ret = picoquic_process_ack_of_stream_frame(cnx, data, length - offset, &frame_length);
                offset += frame_length;
                if (p->send_path != NULL)
                {
...
static int picoquic_process_ack_range(
    quic_cnx_t* cnx, picoquic_packet_context_enum pc, picoquic_packet_context_t* pkt_ctx,
    uint64_t highest, uint64_t range, quic_packet_t** ppacket,
    uint64_t current_time, picoquic_packet_data_t* packet_data)
{
    quic_packet_t* p = *ppacket;
    int ret = 0;
    ...
    while (p != NULL && range > 0)
    {
        ...
        else
        {
            if (p->sequence_number == highest)
            {
                quic_packet_t* next = p->packet_previous;
                picoquic_path_t* old_path = p->send_path;
                ...
                if (old_path != NULL)
                {
                    int packet_len = p->length;
                    old_path->delivered += packet_len;
                    ...
                    if ((packet_len + p->checksum_overhead) == old_path->send_mtu)
                    {
                        old_path->nb_mtu_losses = 0;
                    }
                    else if ((packet_len + p->checksum_overhead) > old_path->send_mtu)
                    {
                        old_path->send_mtu = packet_len + p->checksum_overhead;
                        old_path->mtu_probe_sent = 0;
                    }
                }
...
const uint8_t* picoquic_decode_ack_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                         const uint8_t* bytes_max, uint64_t current_time, int epoch, int is_ecn,
                                         int has_path_id, picoquic_packet_data_t* packet_data)
{
    uint64_t path_id = 0;
    uint64_t num_block;
    uint64_t largest;
    ...
    uint64_t ftype = is_ecn ? picoquic_frame_type_ack_ecn : picoquic_frame_type_ack;
    ...
    else
    {
        ...
        else
        {
            ...
            int is_new_ack = 0;
            quic_packet_t* top_packet =
                picoquic_find_acked_packet(cnx, pkt_ctx, largest, current_time, &is_new_ack);
            quic_packet_t* p_retransmitted_previous = pkt_ctx->retransmitted_newest;
            ...
            do
            {
                ...
                if (picoquic_process_ack_range(cnx, pc, pkt_ctx, largest, range, &top_packet, current_time,
                                               packet_data) != 0)
                {
                    bytes = NULL;
                    break;
    ...
    if (bytes != 0 && is_ecn)
    {
        ...
        if (ecnx3[2] > pkt_ctx->ecn_ce_total_remote)
        {
            ...
            quic_congestion_notify(cnx, congestion_notification_ecn_ec,
                                   0, current_time);
        }
    }
...
uint8_t* picoquic_format_ack_frame_in_context(quic_cnx_t* cnx, uint8_t* bytes, uint8_t* bytes_max,
                                              int* more_data, uint64_t current_time, picoquic_ack_context_t* ack_ctx,
                                              int* need_time_stamp,
                                              uint64_t multipath_sequence, int is_opportunistic)
{
    ...
    uint64_t ack_type_byte = (is_ecn) ? picoquic_frame_type_ack_ecn : picoquic_frame_type_ack;
...
uint8_t* picoquic_format_ack_frame(quic_cnx_t* cnx, uint8_t* bytes, uint8_t* bytes_max,
                                   ...
                                   int is_opportunistic)
{
        ...
        for (int path_id = 0; path_id < cnx->nb_paths; path_id++)
        {
            if (bytes != NULL)
            {
                ...
                bytes = picoquic_format_ack_frame_in_context(cnx, bytes, bytes_max, more_data,
                                                             current_time, ack_ctx, &need_time_stamp,
                                                             cnx->path[path_id]->unique_path_id, is_opportunistic);
                if (is_opportunistic)
                {
    ...
    {
        bytes = picoquic_format_ack_frame_in_context(cnx, bytes, bytes_max, more_data,
                                                     current_time, &cnx->ack_ctx[pc], &need_time_stamp, UINT64_MAX,
                                                     is_opportunistic);
    }
...
uint64_t picoquic_ack_gap_override_if_needed(quic_cnx_t* cnx, int path_index)
{
...
int picoquic_is_ack_needed_in_ctx(quic_cnx_t* cnx, picoquic_ack_context_t* ack_ctx, uint64_t current_time,
                                  int path_index, uint64_t* next_wake_time, picoquic_packet_context_enum pc,
                                  int is_opportunistic)
{
    int ret = 0;
    ...
    if (ack_ctx->act[is_opportunistic].ack_needed)
    {
        ...
        else
        {
            ...
            if (ack_ctx->act[is_opportunistic].highest_ack_sent + ack_gap <= picoquic_sack_list_last(
                    &ack_ctx->sack_list) ||
                ack_ctx->act[is_opportunistic].time_oldest_unack_packet_received + cnx->ack_delay_remote <=
                current_time)
            {
                ret = 1;
            }
            ...
            {
                if (ack_ctx->act[is_opportunistic].time_oldest_unack_packet_received + cnx->ack_delay_remote < *
                    next_wake_time)
                {
                    *next_wake_time = ack_ctx->act[is_opportunistic].time_oldest_unack_packet_received + cnx->
                        ack_delay_remote;
...
uint8_t* picoquic_format_connection_close_frame(quic_cnx_t* cnx,
                                                uint8_t* bytes, uint8_t* bytes_max, int* more_data, int* is_pure_ack)
{
    uint8_t* bytes0 = bytes;
...
const uint8_t* picoquic_decode_connection_close_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                      const uint8_t* bytes_max)
{
    ...
    else
    {
        ...
        cnx->cnx_state = (cnx->cnx_state < picoquic_state_client_ready_start || cnx->crypto_context[picoquic_epoch_1rtt]
                             .aead_decrypt == NULL)
...
uint8_t* picoquic_format_application_close_frame(quic_cnx_t* cnx,
                                                 uint8_t* bytes, uint8_t* bytes_max, int* more_data, int* is_pure_ack)
{
    uint8_t* bytes0 = bytes;
...
const uint8_t* picoquic_decode_application_close_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                       const uint8_t* bytes_max)
{
    ...
    {
        cnx->cnx_state = (cnx->cnx_state < picoquic_state_client_ready_start)
                             ? picoquic_state_disconnected
                             : picoquic_state_closing_received;
        // if (cnx->callback_fn)
        // {
...
uint8_t* picoquic_format_max_data_frame(quic_cnx_t* cnx, uint8_t* bytes, uint8_t* bytes_max,
...
const uint8_t* picoquic_decode_max_data_frame(quic_cnx_t* cnx, const uint8_t* bytes, const uint8_t* bytes_max)
{
    uint64_t maxdata;
...
uint8_t* picoquic_format_max_stream_data_frame(quic_cnx_t* cnx, quic_stream_t* stream,
                                               uint8_t* bytes, uint8_t* bytes_max, int* more_data, int* is_pure_ack,
...
const uint8_t* picoquic_decode_max_stream_data_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                     const uint8_t* bytes_max)
{
    uint64_t stream_id;
    uint64_t maxdata = 0;
...
uint8_t* picoquic_format_required_max_stream_data_frames(quic_cnx_t* cnx,
                                                         uint8_t* bytes, uint8_t* bytes_max, int* more_data,
                                                         int* is_pure_ack)
{
    uint8_t* bytes0;
    quic_stream_t* stream = picoquic_first_stream(cnx);
    ...
    while (stream != NULL)
    {
        if (!stream->fin_received)
        {
            ...
            if (!stream->socket.receive_reset && 2 * stream->consumed_offset > stream->maxdata_local)
            {
                bytes0 = bytes;

                if ((bytes = picoquic_format_max_stream_data_frame(cnx, stream, bytes, bytes_max, more_data,
                                                                   is_pure_ack,
                                                                   stream->maxdata_local + new_window)) == bytes0)
                {
...
uint8_t* picoquic_format_max_streams_frame_if_needed(quic_cnx_t* cnx,
                                                     uint8_t* bytes, uint8_t* bytes_max, int* more_data,
                                                     int* is_pure_ack)
{
    uint8_t* bytes0 = bytes;
    ...
    {
        uint64_t new_unidir_local = cnx->max_stream_id_unidir_local + 4 * cnx->local_parameters.
...
void picoquic_update_max_stream_ID_local(quic_cnx_t* cnx, quic_stream_t* stream)
{
    if (cnx->client_mode != IS_CLIENT_STREAM_ID(stream->stream_id) && !stream->max_stream_updated)
    {
        ...
        if (stream->consumed_offset >= stream->fin_offset && (stream->fin_received || stream->socket.receive_reset))
        {
            /* Receive is complete */
            if (IS_BIDIR_STREAM_ID(stream->stream_id))
            {
...
const uint8_t* picoquic_decode_max_streams_frame(quic_cnx_t* cnx, const uint8_t* bytes, const uint8_t* bytes_max,
                                                 int max_streams_frame_type)
{
    uint64_t max_stream_rank;
...
const uint8_t* picoquic_decode_path_challenge_frame(quic_cnx_t* cnx, const uint8_t* bytes, const uint8_t* bytes_max,
                                                    picoquic_path_t* path_x, fsockaddr_t* addr_from,
                                                    fsockaddr_t* addr_to)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (!is_valid)
            {
                ...
                {
                    if (addr_from == NULL ||
                        fsockaddr_compare(addr_from, &tuple->peer_addr) == 0)
                    {
                        ...
                        */
                        if (addr_to == NULL ||
                            (tuple->local_addr.port == 0 &&
                                addr_to->ip == tuple->local_addr.ip) ||
                            fsockaddr_compare(addr_to, (fsockaddr_t*)&tuple->local_addr) == 0)
                        {
                            is_valid = 1;
            ...
            if (!is_valid)
            {
                ...
                picoquic_log_app_message(cnx,
                                         "Path challenge[%" PRIu64
                                         "] from %s to %s ignored, wrong addresses, expected %s - %s.\n",
                                         path_x->unique_path_id,
                                         picoquic_addr_text(addr_from, buf1, sizeof(buf1)),
                                         picoquic_addr_text(addr_to, buf2, sizeof(buf2)),
                                         picoquic_addr_text((fsockaddr_t*)&path_x->first_tuple->peer_addr, buf3,
                                                            sizeof(buf3)),
                                         picoquic_addr_text((fsockaddr_t*)&path_x->first_tuple->local_addr, buf4,
                                                            sizeof(buf4))
                );
            }
                ...
                * and match the preferred address.
                ...
                if (cnx->local_parameters.prefered_address.is_defined)
                {
                    ...
                    picoquic_log_app_message(
                        cnx, "Enabling migration after preferred address validation on path %" PRIu64,
                        path_x->unique_path_id);
                }
                else
                {
                    picoquic_connection_error_ex(cnx, PICOQUIC_TRANSPORT_PROTOCOL_VIOLATION,
                                                 picoquic_frame_type_path_challenge, "Migration is disabled");
                }
            }
        }
    }
...
const uint8_t* picoquic_decode_path_response_frame(quic_cnx_t* cnx, const uint8_t* bytes, const uint8_t* bytes_max,
                ...
                */
                if (previous_tuple != NULL /* && cnx->client_mode */)
                {
                    ...
                    if (tuple->to_preferred_address)
                    {
                        ...
                        picoquic_log_app_message(
                            cnx, "Migration to server preferred address successful on path %" PRIu64,
                            path_x->unique_path_id);
                    }
...
int picoquic_should_repeat_path_response_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                               size_t bytes_max)
{
    /* On the client side, challenge responses generally ought to be repeated in order to maximise
    * chances of handshake success. However, doing so on the server side may create a "blowback"
...
const uint8_t* picoquic_decode_blocked_frame(quic_cnx_t* cnx, const uint8_t* bytes, const uint8_t* bytes_max)
{
    if ((bytes = picoquic_frames_varint_skip(bytes + 1, bytes_max)) == NULL)
    {
        picoquic_connection_error(cnx, PICOQUIC_TRANSPORT_FRAME_FORMAT_ERROR,
...
const uint8_t* picoquic_decode_stream_blocked_frame(quic_cnx_t* cnx, const uint8_t* bytes, const uint8_t* bytes_max)
{
...
const uint8_t* picoquic_decode_streams_blocked_frame(quic_cnx_t* cnx, const uint8_t* bytes,
                                                     const uint8_t* bytes_max, uint8_t frame_id)
{
    uint64_t stream_limit = 0;
 ...
 */
const uint8_t* picoquic_decode_handshake_done_frame(quic_cnx_t* cnx, const uint8_t* bytes, uint64_t current_time)
{
    if (!cnx->client_mode)
    {
...
int quic_enqueue_handshake_done_frame(quic_cnx_t* cnx)
{
    ...
    return picoquic_queue_misc_frame(cnx, &frame_buffer, 1, 0, picoquic_packet_context_application);
}

uint8_t* picoquic_format_first_datagram_frame(quic_cnx_t* cnx, uint8_t* bytes,
                                              uint8_t* bytes_max, int* more_data, int* is_pure_ack)
{
...
{
    picoquic_remote_cnxid_stash_t* stash = picoquic_find_or_create_remote_cnxid_stash(
        path_x->cnx, path_x->unique_path_id, 0);
                                                 ...
                                                 const uint8_t* bytes_max, int* no_need_to_repeat)
{
    ...
    *no_need_to_repeat = 0;
    ...
    if ((bytes = picoquic_parse_path_cid_blocked_frame(bytes, bytes_max, &unique_path_id, &next_sequence_number)) ==
        NULL)
    {
        /* Malformed frame, do not retransmit */
        *no_need_to_repeat = 1;
...
int picoquic_decode_frames(quic_cnx_t* cnx, picoquic_path_t* path_x,
                           quic_packet_t* packet, int epoch,
                           fsockaddr_t* addr_from, fsockaddr_t* addr_to,
                           uint64_t pn64, int path_is_not_allocated,
                           uint64_t current_time)
{
    const uint8_t* bytes = packet->bytes + packet->header_length;
    size_t bytes_maxsize = packet->length - packet->header_length;
    ...
    while (bytes != NULL && bytes < bytes_max)
    {
        ...
        if (PICOQUIC_IN_RANGE(first_byte, picoquic_frame_type_stream_range_min, picoquic_frame_type_stream_range_max))
        {
            ...
            bytes = picoquic_decode_stream_frame(cnx, packet, bytes, bytes_max, current_time);
            ack_needed = 1;
        }
        ...
        else if (epoch != picoquic_epoch_0rtt && epoch != picoquic_epoch_1rtt && first_byte !=
            picoquic_frame_type_padding
        ...
        else
        {
            ...
            case picoquic_frame_type_path_challenge:
                ...
                bytes = picoquic_decode_path_challenge_frame(cnx, bytes, bytes_max,
                                                             (path_is_not_allocated) ? NULL : path_x, addr_from,
                                                             addr_to);
                break;
            case picoquic_frame_type_path_response:
                is_path_probing_frame = 1;
                ...
                break;
            case picoquic_frame_type_crypto_hs:
                bytes = picoquic_decode_crypto_hs_frame(cnx, packet, bytes, bytes_max, epoch);
                ack_needed = 1;
                break;
            case picoquic_frame_type_new_token:
...
// 跳过（解析时忽略）QUIC 数据包中的一个帧，并返回下一个帧的起始位置，同时告知已消耗的字节数和该帧是否为纯ACK帧。
