...
uint64_t picoquic_transport_param_varint_decode(quic_cnx_t* cnx, uint8_t* bytes, uint64_t extension_length,
                                                int* ret)
{
    uint64_t n64 = 0;
...
uint8_t* picoquic_transport_param_type_varint_encode(uint8_t* bytes, const uint8_t* bytes_max, picoquic_tp_enum tp_type,
                                                     uint64_t n64)
{
    if (bytes != NULL &&
...
uint8_t* picoquic_transport_param_cid_encode(uint8_t* bytes, const uint8_t* bytes_max, picoquic_tp_enum tp_type,
...
int picoquic_transport_param_cid_decode(quic_cnx_t* cnx, uint8_t* bytes, uint64_t extension_length,
                                        quic_connection_id_t* cid)
{
    int ret = 0;
...
uint8_t* picoquic_encode_transport_param_version_negotiation(uint8_t* bytes, uint8_t* bytes_max,
                                                             int extension_mode, quic_cnx_t* cnx)
{
    uint8_t* bytes_len;
                                               ...
                                               picoquic_supported_versions[cnx->version_index].version)) != NULL)
    {
        if (extension_mode == 0)
        {
            if (cnx->desired_version != 0 && cnx->desired_version != picoquic_supported_versions[cnx->version_index].
                version)
            {
...
const uint8_t* picoquic_process_tp_version_negotiation(const uint8_t* bytes, const uint8_t* bytes_max,
                                                       ...
                                                       int* negotiated_index, uint64_t* vn_error)
{
    uint32_t current;
...
int picoquic_negotiate_multipath_option(quic_cnx_t* cnx)
{
    int ret = 0;
                                          ...
                                          uint8_t* bytes, size_t bytes_length, size_t* consumed)
{
    ...
    if (cnx->local_parameters.max_ack_delay != PICOQUIC_ACK_DELAY_MAX_DEFAULT)
    {
        bytes = picoquic_transport_param_type_varint_encode(bytes, bytes_max, picoquic_tp_max_ack_delay,
                                                            (cnx->local_parameters.max_ack_delay + 999) / 1000);
        /* Max ACK delay in milliseconds */
    }
    bytes = picoquic_transport_param_cid_encode(bytes, bytes_max, picoquic_tp_handshake_connection_id,
    ...
    if (extension_mode == 1)
    {
        ...
        {
            bytes = picoquic_transport_param_cid_encode(bytes, bytes_max, picoquic_tp_original_connection_id,
                                                        &cnx->original_cnxid);
            bytes = picoquic_transport_param_cid_encode(bytes, bytes_max, picoquic_tp_retry_connection_id,
                                                        &cnx->initial_cnxid);
        }
    ...
    if (extension_mode == 1)
    {
            ...
            (bytes = picoquic_frames_varint_encode(bytes, bytes_max, PICOQUIC_RESET_SECRET_SIZE)) != NULL)
        {
            ...
            {
                (void)picoquic_create_cnxid_reset_secret(cnx->quic, &cnx->path[0]->first_tuple->p_local_cnxid->cnx_id,
                                                         bytes);
    ...
    if (cnx->local_parameters.address_discovery_mode > 0 && bytes != NULL)
    {
        bytes = picoquic_transport_param_type_varint_encode(bytes, bytes_max,
                                                            ...
                                                            (uint64_t)(cnx->local_parameters.address_discovery_mode -
                                                                1));
    }
...
void picoquic_clear_transport_extensions(quic_cnx_t* cnx)
{
                                          ...
                                          uint8_t* bytes, size_t bytes_max, size_t* consumed)
{
    ...
    quic_connection_id_t original_connection_id = picoquic_null_connection_id;
    quic_connection_id_t handshake_connection_id = picoquic_null_connection_id;
    quic_connection_id_t retry_connection_id = picoquic_null_connection_id;
            ...
            else
            {
                ...
                */
                ...
                case picoquic_tp_initial_max_streams_bidi:
                    {
                        ...
                        cnx->max_stream_id_bidir_remote = STREAM_ID_FROM_RANK(
                            (cnx->remote_parameters.initial_max_stream_id_bidir == 0xFFFFFFFF) ? 0 : cnx->
                            remote_parameters.initial_max_stream_id_bidir,
                            cnx->client_mode, 0);
                ...
                case picoquic_tp_max_packet_size:
                    {
                        ...
                        uint64_t max_packet_size = picoquic_transport_param_varint_decode(
                            cnx, bytes + byte_index, extension_length, &ret);
                        if (ret == 0)
                        {
                ...
                case picoquic_tp_stateless_reset_token:
                    ...
                    {
                        memcpy(cnx->path[0]->first_tuple->p_remote_cnxid->reset_secret, bytes + byte_index,
                ...
                case picoquic_tp_initial_max_streams_uni:
                    {
                        ...
                        cnx->max_stream_id_unidir_remote = STREAM_ID_FROM_RANK(
                            (cnx->remote_parameters.initial_max_stream_id_unidir == 0xFFFFFFFF) ? 0 : cnx->
                            remote_parameters.initial_max_stream_id_unidir,
                            cnx->client_mode, 1);
                ...
                case picoquic_tp_original_connection_id:
                    ret = picoquic_transport_param_cid_decode(cnx, bytes + byte_index, extension_length,
                ...
                case picoquic_tp_handshake_connection_id:
                    ...
                    if (ret == 0)
                    {
                        if (picoquic_compare_connection_id(&cnx->path[0]->first_tuple->p_remote_cnxid->cnx_id,
                ...
                /* Values of 0 and values larger that 2^24 are not expected */
                    ...
                    {
                        ret = picoquic_connection_error_ex(cnx, PICOQUIC_TRANSPORT_PROTOCOL_VIOLATION, 0,
                                                           "Min ack delay TP");
                    }
    ...
    cnx->is_loss_bit_enabled_outgoing = (cnx->local_parameters.enable_loss_bit > 1) && (cnx->remote_parameters.
        enable_loss_bit > 0);
    cnx->is_loss_bit_enabled_incoming = (cnx->local_parameters.enable_loss_bit > 0) && (cnx->remote_parameters.
        enable_loss_bit > 1);
    ...
    else
    {
        ...
        cnx->local_parameters.do_grease_quic_bit = cnx->remote_parameters.do_grease_quic_bit && !cnx->quic->
            one_way_grease_quic_bit;
