...
static void picoquic_validate_bdp_seed(quic_cnx_t* cnx, picoquic_path_t* path_x, uint64_t rtt_sample,
                                       uint64_t current_time)
{
        ...
        !cnx->cwin_notified_from_seed)
    {
            ...
            cnx->seed_rtt_min <= rtt_sample + rtt_margin)
        {
            ...
            picoquic_get_ip_addr(&path_x->first_tuple->peer_addr, &ip_addr, &ip_addr_length);
                ...
                memcmp(ip_addr, cnx->seed_ip_addr, ip_addr_length) == 0)
            {
                ...
                quic_congestion_notify(cnx, congestion_notification_seed_cwin,
...
void picoquic_update_path_rtt_one_way(quic_cnx_t* cnx, picoquic_path_t* old_path, picoquic_path_t* path_x,
                                      uint64_t send_time, uint64_t current_time, uint64_t ack_delay,
                                      uint64_t time_stamp)
{
    if (time_stamp != 0)
    {
