...
#define PICOQUIC_ERROR_CLASS -1000
...
typedef enum
{
    picoquic_state_client_init, //客户端cnx刚创建
    picoquic_state_client_init_sent, //客户端cnx发送了initial包
    picoquic_state_client_renegotiate, // 客户端cnx需要重新协商
    picoquic_state_client_retry_received, //客户端cnx收到了retry包
    picoquic_state_client_init_resent, //客户端cnx重发了initial包
    picoquic_state_server_init, //服务端cnx刚创建, 还未开始握手
    picoquic_state_server_handshake, // 服务端收到客户端Initial，发送完ServerHello，进入该状态
    picoquic_state_client_handshake_start, //收到 ServerHello，客户端进入握手阶段
    picoquic_state_handshake_failure, //握手失败
    picoquic_state_handshake_failure_resend, //握手失败，重发握手包
    picoquic_state_client_almost_ready, // 已发送Handshake包（Finished），等待服务端的确认，还不能发送数据
    picoquic_state_server_false_start, // 服务端“假启动”，为了支持接收0-RTT数，此状态服务端可提前接收0-RTT数据在握手未完全结束时。
    picoquic_state_server_almost_ready, // 服务端cnx握手即将完成，等待客户端的确认
    picoquic_state_client_ready_start, // 客户端cnx准备就绪，等待服务端ready(如服务端的应用数据包或特定ACK), 可以发送数据
    picoquic_state_ready, // 客户端和服务端cnx都已准备就绪，握手完成
    picoquic_state_disconnecting, // 客户端或服务端cnx主动断开连接,
    picoquic_state_closing_received, // 客户端或服务端cnx收到了关闭连接的请求
    picoquic_state_closing, // 客户端或服务端cnx正在关闭连接
    picoquic_state_draining, // 客户端或服务端cnx正在排空数据
    picoquic_state_disconnected // 客户端或服务端cnx已断开连接
} picoquic_state_enum;
...
typedef struct quic_connection_id
{
    uint8_t id[PICOQUIC_CONNECTION_ID_MAX_SIZE];
    uint8_t id_len;
} quic_connection_id_t;
...
typedef struct st_quic_context quic_context_t;
typedef struct st_quic_cnx_t quic_cnx_t;
...
typedef struct st_picoquic_tp_prefered_address_t
{
    ...
    uint16_t ipv6Port;
    quic_connection_id_t connection_id;
...
uint64_t picoquic_get_quic_time(quic_context_t* quic); /* connection time, compatible with simulations */
 ...
 */
typedef int (*picoquic_stream_data_cb_fn)(quic_cnx_t* cnx,
 ...
 */
typedef size_t (*picoquic_alpn_select_fn)(quic_context_t* quic, ptls_iovec_t* list, size_t count);
...
typedef void (*picoquic_connection_id_cb_fn)(quic_context_t* quic, quic_connection_id_t cnx_id_local,
                                             quic_connection_id_t cnx_id_remote, void* cnx_id_cb_data,
                                             quic_connection_id_t* cnx_id_returned);
 ...
 */
typedef uint32_t (*picoquic_fuzz_fn)(void* fuzz_ctx, quic_cnx_t* cnx, uint8_t* bytes,
 ...
 */
void picoquic_log_app_message_v(quic_cnx_t* cnx, const char* fmt, va_list vargs);
void picoquic_log_app_message(quic_cnx_t* cnx, const char* fmt, ...);
 ...
 */
typedef int (*picoquic_verify_certificate_cb_fn)(void* ctx, quic_cnx_t* cnx, ptls_iovec_t* certs, size_t num_certs,
...
bool quic_check_port_blocked(uint16_t port);
...
quic_context_t* picoquic_create(uint32_t max_nb_connections,
...
int picoquic_set_tls_key(quic_context_t* quic, const uint8_t* data, size_t len);
 ...
 */
int picoquic_load_retry_tokens(quic_context_t* quic, char const* token_store_filename);
int picoquic_save_session_tickets(quic_context_t* quic, char const* ticket_store_filename);
int picoquic_save_retry_tokens(quic_context_t* quic, char const* token_store_filename);

/* Manage bdps */
void picoquic_set_default_bdp_frame_option(quic_context_t* quic, int enable_bdp_frame);
...
#define PICOQUIC_MTU_OVERHEAD(addr) (((addr)->family==FSOCKADDR_IPV4)?28:48)
 ...
 */
void picoquic_set_default_callback(quic_context_t* quic, picoquic_stream_data_cb_fn callback_fn, void* callback_ctx);
...
int picoquic_abandon_path(quic_cnx_t* cnx, uint64_t unique_path_id,
...
int picoquic_set_path_status(quic_cnx_t* cnx, uint64_t unique_path_id, picoquic_path_status_enum status);
...
int picoquic_set_local_addr(quic_cnx_t* cnx, fsockaddr_t* addr);
...
void picoquic_set_callback(quic_cnx_t* cnx,
                           picoquic_stream_data_cb_fn callback_fn, void* callback_ctx);

picoquic_stream_data_cb_fn picoquic_get_default_callback_function(quic_context_t* quic);

void* picoquic_get_default_callback_context(quic_context_t* quic);

picoquic_stream_data_cb_fn picoquic_get_callback_function(quic_cnx_t* cnx);

void* picoquic_get_callback_context(quic_cnx_t* cnx);
...
int picoquic_queue_datagram_frame(quic_cnx_t* cnx, size_t length, const uint8_t* bytes);
 ...
 * The port numbers in the socket addresses structures are expressed in network order.
 */
void picoquic_notify_destination_unreachable(quic_cnx_t* cnx,
...
typedef int (*picoquic_stream_direct_receive_fn)(quic_cnx_t* cnx,
                                                 uint64_t stream_id, int fin, const uint8_t* bytes, uint64_t offset,
                                                 size_t length,
                                                 void* direct_receive_ctx);

int picoquic_mark_direct_receive_stream(quic_cnx_t* cnx,
                                        uint64_t stream_id, picoquic_stream_direct_receive_fn direct_receive_fn,
                                        void* direct_receive_ctx);

/* Associate stream with app context */
int picoquic_set_app_stream_ctx(quic_cnx_t* cnx,
                                uint64_t stream_id, void* app_stream_ctx);
/* Remove association between stream and context */
void picoquic_unlink_app_stream_ctx(quic_cnx_t* cnx, uint64_t stream_id);
 ...
 */
int picoquic_mark_active_stream(quic_cnx_t* cnx,
...
int picoquic_mark_high_priority_stream(quic_cnx_t* cnx,
...
void picoquic_set_default_datagram_priority(quic_context_t* quic, uint8_t default_datagram_priority);
 ...
 */
int picoquic_add_to_stream(quic_cnx_t* cnx,
                           uint64_t stream_id, const uint8_t* data, size_t length, int set_fin);

void picoquic_reset_stream_ctx(quic_cnx_t* cnx, uint64_t stream_id);
 ...
 */
int picoquic_add_to_stream_with_ctx(quic_cnx_t* cnx, uint64_t stream_id, const uint8_t* data, size_t length,
                                    int set_fin, void* app_stream_ctx);
...
typedef struct st_picoquic_per_ack_state_t
{
    ...
    /* Number of bytes acked between the time the packet was sent and now */
